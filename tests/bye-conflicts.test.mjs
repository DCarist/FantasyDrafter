...
eq(
  L.getByeClashStatus(null, []),
  { type: 'none', samePos: [], otherPos: [] },
  'Null candidate returns type=none',
);
eq(
  L.getByeClashStatus({ pos: 'QB', bye: null }, [{ name: 'Josh Allen', pos: 'QB', bye: 7 }]),
  { type: 'none', samePos: [], otherPos: [] },
  'Candidate with null bye returns type=none',
);
eq(
  L.getByeClashStatus({ pos: 'QB', bye: 7 }, []),
  { type: 'none', samePos: [], otherPos: [] },
  'Empty roster returns type=none',
);

// --- 2. Direct same-position bye clash ---
const rosterWithAllen = [{ id: 0, name: 'Josh Allen', pos: 'QB', bye: 7 }];
...
eq(
  ceedeClash.type,
  'other-pos',
  'CeeDee Lamb (WR, Bye 7) has other-pos clash with Josh Allen (QB, Bye 7)',
);
...
eq(
  wr7Status.samePos.map((p) => p.name),
  ['CeeDee Lamb'],
  'samePos list contains CeeDee Lamb',
);
eq(
  wr7Status.otherPos.map((p) => p.name),
  ['Josh Allen'],
  'otherPos list contains Josh Allen',
);
...
eq(
  qb10Status.otherPos.map((p) => p.name),
  ['Justin Jefferson', 'Unlisted Receiver'],
  'Lists both cross-position receivers',
);
...
eq(
  testClash.type,
  'same-pos',
  'Terry McLaurin (WR, Bye 9) has same-pos clash with custom Ray-Ray McCloud (WR, Bye 9)',
);
