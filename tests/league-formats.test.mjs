...
assert(
  L.FORMAT_OPTIONS.scoring.ppr && L.FORMAT_OPTIONS.scoring.half && L.FORMAT_OPTIONS.scoring.std,
  'Includes PPR, Half-PPR, STD scoring modes',
);
...
eq(
  L.getRedraftRank(jamarrChase, '1qb', 'half'),
  3.0,
  "Ja'Marr Chase 1QB Half-PPR redraft rank is 3.0",
);
eq(
  L.getRedraftRank(jamarrChase, '1qb', 'std'),
  2.0,
  "Ja'Marr Chase 1QB Standard redraft rank is 2.0",
);
eq(L.getRedraftRank(jamarrChase, 'sf', 'ppr'), 6.0, "Ja'Marr Chase SF PPR redraft rank is 6.0");

// 4. computeFormatScore Calculations
const allenSFScore = L.computeFormatScore(joshAllen, {
  qbFormat: 'sf',
  scoring: 'half',
  blend: 0.6,
});
const allen1QBScore = L.computeFormatScore(joshAllen, {
  qbFormat: '1qb',
  scoring: 'half',
  blend: 0.6,
});
assert(
  allenSFScore > allen1QBScore + 10,
  'Josh Allen draft score is significantly higher in Superflex than 1QB',
);

const chase1QBScore = L.computeFormatScore(jamarrChase, {
  qbFormat: '1qb',
  scoring: 'ppr',
  blend: 0.6,
});
const chaseSFScore = L.computeFormatScore(jamarrChase, {
  qbFormat: 'sf',
  scoring: 'ppr',
  blend: 0.6,
});
...
const bowersStandard = L.computeFormatScore(brockBowers, {
  qbFormat: 'sf',
  scoring: 'half',
  tePremium: false,
});
const bowersPremium = L.computeFormatScore(brockBowers, {
  qbFormat: 'sf',
  scoring: 'half',
  tePremium: true,
});
eq(
  Math.round(bowersPremium * 100),
  Math.round(bowersStandard * 1.08 * 100),
  'TE Premium applies exact 1.08x boost to TE score',
);

const chaseStandard = L.computeFormatScore(jamarrChase, {
  qbFormat: 'sf',
  scoring: 'half',
  tePremium: false,
});
const chasePremium = L.computeFormatScore(jamarrChase, {
  qbFormat: 'sf',
  scoring: 'half',
  tePremium: true,
});
...
eq(
  L.getProspectRank(jeremiyahLove, 'sf'),
  24.0,
  'Jeremiyah Love resolves to #24 overall in Superflex dynasty',
);

eq(L.getProspectRank(fernandoMendoza, '1qb'), 8, 'Fernando Mendoza rookie draft rank is #8 in 1QB');
eq(
  L.getProspectRank(fernandoMendoza, 'sf'),
  53.0,
  'Fernando Mendoza resolves to #53 in Superflex dynasty (big jump for rookie QB)',
);
