...
eq(
  L.formatLineupSummary(defaultSlots),
  'QB · 2RB · 2WR · TE · 3FLEX · SF · 15 BN',
  'Formats standard superflex lineup',
);
...
eq(
  L.formatLineupSummary(customSlots),
  '2QB · 2RB · 3WR · TE · 2FLEX · K · D/ST · 8 BN',
  'Formats 2QB + K + DST lineup',
);
...
eq(
  emptyRoster.starters.every((s) => s.player === null),
  true,
  'All starter slots are open when no players drafted',
);
...
eq(
  flexStarters.map((s) => s.player.name),
  ['Amon-Ra St. Brown', 'Garrett Wilson', 'Breece Hall'],
  'Flex slots filled with remaining RB/WRs',
);
...
eq(
  filledRoster.bench.map((b) => b.player.name),
  ['Jayden Daniels', 'Malik Nabers'],
  'Bench contains Daniels and Nabers',
);
...
eq(
  wrHeavyRoster.starters.filter((s) => s.player !== null).length,
  6,
  '6 WRs fill 2 WR + 3 FLEX + 1 SF',
);
eq(
  wrHeavyRoster.starters.find((s) => s.slotType === 'SF').player.name,
  'WR 6',
  'Superflex filled by WR when no QB drafted',
);
...
eq(
  defAliasRoster.starters.find((s) => s.slotType === 'DST').player.name,
  'Dallas DEF',
  'Recognizes DEF alias for D/ST starter slot',
);
...
eq(
  L.formatLineupSummary(zeroBenchSlots),
  'QB · 2RB · 2WR · TE · FLEX',
  'Formats lineup without bench suffix when bench is 0',
);
eq(
  L.formatLineupSummary({
    qb: 0,
    rb: 0,
    wr: 0,
    te: 0,
...
assert(
  emptyQbHtml.includes('<span class="slot-label-tag">[QB]</span>'),
  'Empty slot renders [QB] tag',
);
...
assert(
  emptySfHtml.includes('<span class="slot-label-tag">[SF]</span>'),
  'Empty Superflex renders [SF] tag',
);
...
assert(
  !filledQbHtml.includes('slot-label-tag'),
  'Filled starter slot does NOT have slot-label-tag [QB]',
);
assert(!filledQbHtml.includes('[QB]'), 'Filled starter slot does NOT have bracketed [QB]');
assert(
  filledQbHtml.includes('<span class="pos QB">QB</span>'),
  'Filled starter slot begins with inline position badge',
);
...
assert(
  !filledBenchHtml.includes('slot-label-tag'),
  'Filled bench slot does NOT have slot-label-tag [BN]',
);
assert(!filledBenchHtml.includes('[BN]'), 'Filled bench slot does NOT have bracketed [BN]');
assert(
  filledBenchHtml.includes('<span class="pos QB">QB</span>'),
  'Filled bench slot renders position badge inline',
);
