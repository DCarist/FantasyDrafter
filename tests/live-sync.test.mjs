...
const parsedSnake = logic.parseSleeperDraft(
  sampleSnakeDraft,
  [{ user_id: 'u1', display_name: 'UserOne' }],
  'u1',
);
...
const resDeebo = logic.resolveRemotePick(
  { name: 'D. Samuel Sr.', pos: 'WR', team: 'SF' },
  fullMockPlayers,
);
eq(resDeebo.playerId, 10, 'Resolves D. Samuel Sr. (WR, SF) to Deebo Samuel Sr.');

const resHerbert = logic.resolveRemotePick(
  { name: 'J. Herbert', pos: 'QB', team: 'LAC' },
  fullMockPlayers,
);
eq(resHerbert.playerId, 11, 'Resolves J. Herbert (QB, LAC) to Justin Herbert');

const resMetcalf = logic.resolveRemotePick(
  { name: 'D. Metcalf', pos: 'WR', team: 'PIT' },
  fullMockPlayers,
);
eq(resMetcalf.playerId, 12, 'Resolves D. Metcalf (WR, PIT) to DK Metcalf');

const resSwift = logic.resolveRemotePick(
  { name: 'D. Swift', pos: 'RB', team: 'CHI' },
  fullMockPlayers,
);
...
const syncResult2 = logic.reconcileDraftLog(
  syncResult1.log,
  remoteFeed,
  mockPlayers,
  initialContext,
...
const syncResult3 = logic.reconcileDraftLog(
  syncResult1.log,
  remoteFeed3,
  mockPlayers,
  initialContext,
...
const syncResult4 = logic.reconcileDraftLog(
  syncResult3.log,
  remoteRollback,
  mockPlayers,
  initialContext,
