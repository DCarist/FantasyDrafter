...
eq(
  stateDefault.settings.leagueType,
  'redraft',
  'normalizeState defaults missing leagueType to redraft',
);

const stateDynasty = { settings: { leagueType: 'dynasty' } };
globalThis.normalizeState(stateDynasty);
eq(
  stateDynasty.settings.leagueType,
  'dynasty',
  'normalizeState preserves valid dynasty leagueType',
);

const stateRedraft = { settings: { leagueType: 'redraft' } };
globalThis.normalizeState(stateRedraft);
eq(
  stateRedraft.settings.leagueType,
  'redraft',
  'normalizeState preserves valid redraft leagueType',
);

const stateInvalid = { settings: { leagueType: 'invalid_type' } };
globalThis.normalizeState(stateInvalid);
eq(
  stateInvalid.settings.leagueType,
  'dynasty',
  'normalizeState sanitizes invalid leagueType to dynasty',
);
...
const redraftStats = L.formatPlayerStats(
  mockPlayer,
  {
    leagueType: 'redraft',
    qbFormat: '1qb',
...
assert(
  redraftLabels.includes('Redraft (1QB HALF)'),
  'Redraft includes matching Redraft (1QB HALF) rank',
);
...
const dynastyStats = L.formatPlayerStats(
  mockRookie,
  {
    leagueType: 'dynasty',
    qbFormat: 'sf',
...
assert(
  dynastyLabels.includes('Rookie Draft Rank'),
  'Dynasty includes Rookie Draft Rank for rookie',
);
...
eq(
  burrowPlayer.name,
  'Joe Burrow',
  'CIN QB1 playerId correctly resolves to Joe Burrow player record',
);
