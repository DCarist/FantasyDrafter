...
const tempDir = mkdtempSync(join(tmpdir(), 'fantasy-espn-sync-'));
const TEST_DB = join(tempDir, 'espn.db');
try {
...
# Exercise cookie authorization and roster import with an in-process ESPN response.
from scripts import espn_client as ec
def entry(pid, name, position, slot):
    return {
        "lineupSlotId": slot,
...
fixture = {
    "members": [
        {"id": swid if i == 3 else f"OWNER-{i}", "displayName": "DougC1995" if i == 3 else f"Owner {i}"}
        for i in range(1, 13)
    ],
    "teams": [
        {
            "id": i, "primaryOwner": swid if i == 3 else f"OWNER-{i}",
            "name": "Corrective and Preventative TDs" if i == 3 else f"Fixture Team {i}",
            "roster": {"entries": [
                entry(4984, "Josh Allen", 1, 0),
                entry(100, "Baltimore Ravens D/ST", 16, 16),
                entry(9228, "Bryce Young", 1, 20),
            ]},
            "record": {"overall": {"wins": i, "losses": 12 - i, "pointsFor": 1000 + i}},
        }
        for i in range(1, 13)
    ],
}
def fixture_espn_text(url, timeout=12, cookie=None):
    assert "/seasons/2026/segments/0/leagues/1960519163?" in url, url
    assert cookie == f"SWID={swid}; espn_s2={espn_s2}", cookie
    return json.dumps(fixture)
ec.fetch_espn_text = fixture_espn_text
...
`;
  ...
  eq(res3.bench_count, 1, 'Lineup slot 20 persists as bench');
  eq(res3.starter_names, ['Josh Allen', 'Baltimore Ravens'], 'QB and D/ST starter names normalize');
  eq(res3.bench_names, ['Bryce Young'], 'Bench player retains its name');
