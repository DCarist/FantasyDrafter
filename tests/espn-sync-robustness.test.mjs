...
function isPlaceholderName(name, teamNames = []) {
  ...
  if (
    /^(on\s*the\s*clock|the\s*clock|clock|drafting|picking|auto\s*pick|autopick|auto|make|make\s*pick|time\s*expired|available|empty|open|player|unknown|skipped|none)$/i.test(
      clean,
    )
  ) {
    return true;
...
function parsePlayerText(rawText, teamNames = []) {
  ...
  clean = clean
    .replace(
      /\b(autopick|drafted|draft|picked|by|round|pick|prk|proj|queue|view|action|status|rost|stats|team|slot|overall)\b/gi,
      ' ',
    )
    .replace(/\s+/g, ' ')
    .trim();
...
eq(
  pJoshAllen.name,
  'Josh Allen',
  'Correctly parses Josh Allen when team named Allen or Josh exists',
);
...
eq(
  pJoshAllenPlain.name,
  'Josh Allen',
  'Correctly parses plain Josh Allen without pos/team when team named Allen exists',
);
...
eq(
  pCaleb.name,
  'Caleb Williams',
  'Correctly parses Caleb Williams when team named Williams exists',
);
...
eq(
  extractPickNumber('4.01', 12),
  37,
  "Extracts overall from '4.01' in 12-team league ((4-1)*12 + 1 = 37)",
);
...
eq(
  extractPickNumber('5.01', 14),
  57,
  "Extracts overall from '5.01' in 14-team league ((5-1)*14 + 1 = 57)",
);
eq(
  extractPickNumber('5.02', 14),
  58,
  "Extracts overall from '5.02' in 14-team league ((5-1)*14 + 2 = 58)",
);
eq(
  extractPickNumber('4.14', 14),
  56,
  "Extracts overall from '4.14' in 14-team league ((4-1)*14 + 14 = 56)",
);
eq(
  extractPickNumber('3.05', 10),
  25,
  "Extracts overall from '3.05' in 10-team league ((3-1)*10 + 5 = 25)",
);
...
const resolvedDaniels = resolveRemotePick(
  { name: 'Jayden Daniels WSH', pos: 'QB', team: 'WSH' },
  PLAYERS,
  { unlistedFallback: true },
);
...
const resolvedTerry = resolveRemotePick(
  { name: 'Terry McLaurin WSH', pos: 'WR', team: 'WSH' },
  PLAYERS,
  { unlistedFallback: true },
);
...
const reconciled = reconcileDraftLog([], burstSnapshot, PLAYERS, {
  teams: 12,
  slot: 1,
  mode: '3rr',
});
...
assert(
  manifestJson.background && manifestJson.background.service_worker === 'background.js',
  'manifest.json registers background.js service worker',
);
assert(
  manifestJson.host_permissions.includes('http://127.0.0.1:8517/*'),
  'manifest.json grants host permissions for 127.0.0.1:8517',
);
...
assert(
  backgroundJsContent.includes('RELAY_REQUEST'),
  'background.js listens for RELAY_REQUEST messages',
);
assert(
  backgroundJsContent.includes('127.0.0.1:8517'),
  'background.js targets local drafter server',
);
...
assert(
  serverPyContent.includes('/api/sync/snapshot'),
  'server.py handles /api/sync/snapshot endpoint',
);
assert(serverPyContent.includes('/api/sync/reset'), 'server.py handles /api/sync/reset endpoint');
assert(
  serverPyContent.includes('server_seen_picks'),
  'server.py maintains server_seen_picks deduplication cache',
);
assert(serverPyContent.includes('latest_snapshot'), 'server.py maintains latest_snapshot state');
assert(
  serverPyContent.includes('"snapshot": latest_snapshot'),
  'server.py returns snapshot in status/poll payload',
);
assert(
  serverPyContent.includes('latest_league_info'),
  'server.py maintains latest_league_info state',
);
assert(
  serverPyContent.includes('"leagueInfo": latest_league_info'),
  'server.py returns leagueInfo in status/poll payload',
);
...
const mockSidebarText =
  'Your draft\nFRESH PRINCE OF...\nYour first pick: Round 1, Pick 5\nPlayers Pick History';
const sidebarMatch = mockSidebarText.match(
  /Your\s*first\s*pick\s*:\s*Round\s*1\s*,\s*Pick\s*(\d+)/i,
);
...
function testApplyLeagueSetup(targetState, info) {
  ...
  if (info.mySlot) {
    targetState.settings.slot = Math.max(
      1,
      Math.min(tCount, parseInt(info.mySlot, 10) || targetState.settings.slot),
    );
  }
  targetState.save();
}
