...
assert(
  !espnClientCode.includes('"User-Agent": "Mozilla/5.0"'),
  'Eliminated naive bare Mozilla/5.0 header',
);
...
assert(
  depthChartsCode.includes('fetch_espn_json'),
  'fetch_depth_charts.py imports and uses fetch_espn_json',
);
assert(
  !depthChartsCode.includes('headers={"User-Agent": "Mozilla/5.0"}'),
  'Naive Mozilla/5.0 removed from fetch_depth_charts.py',
);
assert(
  depthChartsCode.includes('existing_depth_charts'),
  'fetch_depth_charts.py supports retaining existing depth charts on failure',
);
assert(
  depthChartsCode.includes('delay_before='),
  'fetch_depth_charts.py implements inter-request pacing',
);
...
assert(
  injuriesCode.includes('fetch_espn_json'),
  'fetch_injuries.py imports and uses fetch_espn_json',
);
assert(
  !injuriesCode.includes('headers={"User-Agent": "Mozilla/5.0"}'),
  'Naive Mozilla/5.0 removed from fetch_injuries.py',
);
assert(
  injuriesCode.includes('Preserving existing injuries'),
  'fetch_injuries.py guards existing data against wipe on fetch failure',
);
...
assert(
  rankingsCode.includes('DEFAULT_NFL_BYES = {'),
  'update_rankings.py defines DEFAULT_NFL_BYES',
);
const nfl32 = [
  'ARI',
  'ATL',
...
assert(
  serverCode.includes('def handle_data_refresh'),
  'server.py defines handle_data_refresh handler',
);
