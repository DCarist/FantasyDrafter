...
const tempDir = mkdtempSync(join(tmpdir(), 'fantasy-player-details-'));
const TEST_DB = join(tempDir, 'dossier.db');
try {
...
# Exercise the deterministic offline data path rather than depending on live 2026 feeds.
def offline_feed(*args, **kwargs):
    raise OSError("offline test fixture")
ec.fetch_espn_json = offline_feed
mgr.urllib.request.urlopen = offline_feed

mgr.init_db(os.environ["TEST_DB"])
mgr.seed_demo_data(db_path=os.environ["TEST_DB"])

# Sync baseline NFL schedules and weekly stats
sched_count = mgr.sync_nfl_schedules(season=2026, db_path=os.environ["TEST_DB"])
logs_count = mgr.sync_nfl_weekly_stats(season=2026, weeks=[1, 2], db_path=os.environ["TEST_DB"])
def_count = mgr.calculate_defensive_rankings(season=2026, db_path=os.environ["TEST_DB"])
...
# The offline fallback marks opponents TBD. Give Week 3 a real matchup so
# the dossier can exercise its next-opponent and defensive-rank behavior.
schedule = json.loads(cur.execute(
    "SELECT schedule_json FROM nfl_team_schedules WHERE season=2026 AND team='TB'"
).fetchone()[0])
schedule[2]["opponent"] = "BUF"
conn.execute(
    "UPDATE nfl_team_schedules SET schedule_json=? WHERE season=2026 AND team='TB'",
    (json.dumps(schedule),),
)
conn.commit()
conn.close()
qb_dossier = mgr.get_player_details("Baker Mayfield", db_path=os.environ["TEST_DB"])
wr_dossier = mgr.get_player_details("CeeDee Lamb", db_path=os.environ["TEST_DB"])
allen_port = mgr.get_player_details("Josh Allen", db_path=os.environ["TEST_DB"])["portfolio"]
bryce_port = mgr.get_player_details("Bryce Young", db_path=os.environ["TEST_DB"])["portfolio"]
...
print(json.dumps({
    ...
    "qb": qb_dossier,
    "wr": wr_dossier,
    "portfolio": {"allen_port": allen_port, "bryce_port": bryce_port}
}))
`;

  const res1 = pyRunner(initScript);
  assert(res1.total_sched === 32, 'NFL schedules contains rows for all 32 teams');
  eq(res1.total_logs, 10, 'Five seeded players have game logs for both fixture weeks');
  eq(res1.total_def, 32 * 4, 'Defensive rankings cover every team and position');

  // ----------------------------------------------------------------------------
  // 2. QB Dossier Engine (Baker Mayfield)
  // ----------------------------------------------------------------------------
  const qbDossier = res1.qb;
  ...
  assert(
    modalbox.className.includes('player-dossier-modal'),
    'Modal has player-dossier-modal class',
  );
  ...
  // 6. Player Names with Apostrophes (Tre' Harris, De'Zhaun Stribling)
  // ----------------------------------------------------------------------------
  const treDossier = {
    player: {
      name: "Tre' Harris",
  ...
  assert(
    modalbox.innerHTML.includes('Overview &amp; Matchup') ||
      modalbox.innerHTML.includes('Overview & Matchup'),
  ...
  assert(
    modalbox.innerHTML.includes("De'Zhaun Stribling"),
    'Modal header displays "De\'Zhaun Stribling"',
  );
  context.closePlayerModal();
  ...
  assert(
    modalbox.innerHTML.includes("Waiver Note: Tre' Harris"),
    'Modal title includes player name',
  );
  assert(
  ...
  eq(
    context.inSeasonState.watchlist[normKey],
    'High priority waiver stash',
    "Watchlist note saved for Tre' Harris",
  );
} finally {
