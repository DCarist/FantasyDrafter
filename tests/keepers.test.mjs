...
assert(
  resExceed.error.includes('maximum of 2 keepers'),
  'Returns informative error message when max keepers exceeded',
);
...
assert(
  resDup.error.includes('already kept by Team 1'),
  'Identifies existing keeper owner in duplicate error message',
);
...
assert(
  !resNoPick.valid,
  'Disallows assigning keeper in round where team owns 0 picks due to trade',
);
...
assert(
  resSlot4Second.valid,
  'Allows Slot 4 second keeper in Round 3 because Slot 4 owns 2 picks in Round 3',
);
...
assert(
  !resSlot4Third.valid,
  'Disallows Slot 4 third keeper in Round 3 when only 2 picks are owned',
);
...
assert(
  rosterHtml.includes('🔒') || rosterHtml.includes('keeper'),
  'Roster slot HTML renders keeper badge indicator',
);
...
assert(
  mixedMap[3] != null && mixedMap[3].playerId === 51,
  'Team 3 Pick 3 is correctly mapped as keeper',
);
...
assert(
  dougRemapped.every((k) => k.slot === 9),
  'Doug keepers moved from Slot 10 to Slot 9',
);
eq(bryanRemapped.length, 2, 'Preserves Bryan keepers count');
assert(
  bryanRemapped.every((k) => k.slot === 10),
  'Bryan keepers moved from Slot 9 to Slot 10',
);
...
assert(
  remappedPickMap[88] != null && remappedPickMap[88].customName === 'Brock Bowers',
  'Doug Rd 8 keeper mapped to Slot 9 pick #88',
);
assert(
  remappedPickMap[105] != null && remappedPickMap[105].customName === 'Josh Allen',
  'Doug Rd 9 keeper mapped to Slot 9 pick #105',
);
...
assert(
  resEditRound.valid,
  'Allows editing keeper round without violating maxKeepers limit on own team',
);
...
assert(
  resEditTeam.valid,
  'Allows editing keeper team slot without duplicate collision against itself',
);
...
const dougAllocation = L.assignRosterSlots(
  dougPreDraftPicks.map((p) => Object.assign({ entry: p.entry }, p.player)),
  { qb: 1, te: 1, flex: 1 },
);
...
assert(
  qbHtml.includes('🔒') && qbHtml.includes('9.09'),
  'Renders keeper lock badge and formatted pick in starter slot',
);
...
const keepersForDoug = [{ id: 'k_rd8', slot: 9, round: 8, customName: 'Josh Allen' }];
...
eq(
  nextDataAt85.nextDraftPick,
  105,
  'Correctly skips pending keeper #88 and gets next draft pick #105',
);
eq(
  nextDataAt85.distanceToNextDraftPick,
  20,
  'Distance is calculated to next active selection (105 - 85 = 20)',
);
...
const mockElements = {
  ...
  modalbox: {
    className: 'modal modal-wide keepers-modal-box',
    innerHTML: '',
...
assert(
  typeof mockGlobal.saveAndCloseKeepersModal === 'function',
  'saveAndCloseKeepersModal is defined on global scope',
);
assert(
  typeof mockGlobal.saveKeepersAndBackToSetup === 'function',
  'saveKeepersAndBackToSetup is defined on global scope',
);
...
assert(
  mockElements.postabs.innerHTML.includes('ALL'),
  'saveAndCloseKeepersModal triggers render()',
);
...
assert(
  mockElements.modalbox.innerHTML.includes('League Setup'),
  'saveKeepersAndBackToSetup navigates back to League Setup',
);
eq(
  mockElements.modalbox.className,
  'modal modal-wide',
  'openLeagueSetup resets modalbox className to modal modal-wide',
);
...
eq(
  mockElements.modalbox.className,
  'modal modal-wide keepers-modal-box',
  'openKeepersModal sets keepers-modal-box class',
);
...
const matchSuffix = L.findPlayerInPool(
  { name: 'Marvin Harrison', pos: 'WR', team: 'ARI' },
  samplePool,
  lookup,
);
...
const matchTraded = L.findPlayerInPool(
...
// DST match
const matchDst = L.findPlayerInPool(
...
// Non-existent player
const matchMissing = L.findPlayerInPool(
  { name: 'Nonexistent Player', pos: 'WR', team: 'FA' },
  samplePool,
  lookup,
);
...
const stateToReconcile = {
  ...
  log: [{ overall: 1, playerId: 0, name: 'Justin Jefferson', pos: 'WR', team: 'MIN', mine: true }],
...
assert(
  mockElements.modalbox.innerHTML.includes('⚠️ (unranked in pool)'),
  'Keeper table displays ⚠️ (unranked in pool) badge for dropped keeper',
);
assert(
  mockElements.modalbox.innerHTML.includes('Nuk Hopkins'),
  'Keeper table displays dropped keeper name',
);
...
eq(
  mockGlobal.state.settings.leagueName,
  'Pre-Refresh Championship League',
  'triggerDataRefresh auto-saves modified league name before refresh',
);
eq(
  mockGlobal.state.settings.teams,
  14,
  'triggerDataRefresh auto-saves modified teams count before refresh',
);
assert(savedCalled, 'global.save() was called prior to server fetch');
assert(
  Boolean(mockSessionStorage.getItem('pendingDataRefreshToast')),
  'pendingDataRefreshToast flag set in sessionStorage',
);
...
assert(
  mockElements.databanner.innerHTML.includes('Rankings Refreshed'),
  'renderBanner displays rankings refreshed banner',
);
assert(
  mockElements.databanner.innerHTML.includes('Keepers and draft picks were safely reconciled'),
  'renderBanner displays reconciliation confirmation',
);
eq(
  mockSessionStorage.getItem('pendingDataRefreshToast'),
  undefined,
  'pendingDataRefreshToast flag was cleared from sessionStorage',
);
