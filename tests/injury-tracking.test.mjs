...
assert(
  ['Q', 'O', 'IR', 'SUSP', 'D', 'P'].includes(sampleInjured.injury.code),
  'Injury code is valid',
);
...
assert(
  modalInjHtml.includes('injury-accordion active inj-theme-q'),
  'Injured accordion has active theme',
);
assert(
  modalInjHtml.includes('Questionable:</b> Ankle (Sprain)'),
  'Injured accordion shows diagnosis',
);
assert(
  modalInjHtml.includes('Est. Return: <b>2026-09-13</b>'),
  'Injured accordion shows estimated return date',
);
...
assert(
  modalHealthyHtml.includes('injury-accordion inactive'),
  'Healthy player has inactive injury-accordion card',
);
assert(
  modalHealthyHtml.includes('No injury reported as of 2026-09-01'),
  'Healthy player states no injury reported with refresh date',
);
