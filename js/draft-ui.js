...
((global) => {
  ...
  function renderHeader() {
    ...
    if (pick > totalPicks) {
      ...
      const nextData =
        typeof getNextDraftPicks === 'function'
          ? getNextDraftPicks(
              s.slot,
              pick,
              s.teams,
          ...
          const items = nextData.upcoming.slice(0, 5).map((p) => {
            ...
            if (k) {
              ...
              return (
                ...
                '">#' +
                p +
                ' <span class="meta" style="color:var(--warn); font-size:11.5px">(🔒 Keeper)</span></span>'
              );
            }
  ...
  function renderTierPills(allScored, taken) {
    ...
    const scarcityInfo =
      typeof getTierScarcity === 'function'
    ...
    let pillsHtml =
      '<button type="button" class="tierpill' +
      (activeTierFilter == null ? ' active' : '') +
      '" onclick="setTierFilter(null)">All <span class="pill-count">(' +
      totalAvail +
      ')</span></button>';
    ...
    for (const t of scarcityInfo.tiers) {
      ...
      pillsHtml +=
        ...
        (t.isScarcity
          ? t.scarcityLabel
          : `Tier ${t.tier}: ${t.remaining} of ${t.total} available`) +
        '">' +
        'T' +
        t.tier +
  ...
  function formatTierPill(tier) {
    if (tier == null || Number.isNaN(tier)) return '<span class="tier-pill tier-none">—</span>';
  ...
  function getPlayerEdge(p, s) {
    ...
    const espnRank =
      (s && s.scoring === 'std' ? (p.espn_std ?? p.espn_ppr) : (p.espn_ppr ?? p.espn_std)) ??
      p.yahoo ??
      null;
  ...
  function renderPool() {
    ...
    if (global.ui.hideOutIR)
      rows = rows.filter((p) => !(p.injury && (p.injury.code === 'O' || p.injury.code === 'IR')));
    if (global.ui.posFilter === 'WATCHLIST')
      rows = rows.filter((p) => isWatched(global.state.watchlist, p.id));
    else if (global.ui.posFilter === 'ROOKIE') rows = rows.filter((p) => p.rookie);
    else if (global.ui.posFilter === 'DST')
      rows = rows.filter((p) => ['DST', 'DEF', 'D/ST'].includes((p.pos || '').toUpperCase()));
    else if (global.ui.posFilter !== 'ALL')
      rows = rows.filter((p) => (p.pos || '').toUpperCase() === global.ui.posFilter);
    ...
    if ($('th_rk')) {
      $('th_rk').textContent =
        global.ui.posFilter === 'WATCHLIST' && global.ui.sort === 'watch' ? 'Priority' : '#';
      $('th_rk').title =
        global.ui.posFilter === 'WATCHLIST' ? 'Drag rows to reorder watchlist priority' : '';
    }
    ...
    const showTiers =
      ...
      !q &&
      global.ui.posFilter !== 'ROOKIE' &&
      global.ui.posFilter !== 'WATCHLIST' &&
      global.ui.tierFilter == null;
    let html = '',
      prevTier = null;
    ...
    rows.slice(0, 250).forEach((p, idx) => {
      ...
      const posClass = ['QB', 'RB', 'WR', 'TE'].includes(p.pos)
        ? p.pos
        : ['DST', 'DEF', 'D/ST'].includes(p.pos)
          ? 'DST'
          : p.pos === 'K'
            ? 'K'
            : 'other';
      ...
      const value =
        !isTaken && edgeInfo && edgeInfo.edge >= 15
          ? ' <span class="valuetag" title="' +
            `+${edgeInfo.edge} Edge vs ESPN`.replace(/"/g, '&quot;') +
            '">+' +
            edgeInfo.edge +
            ' vs ESPN</span>'
          : !isTaken && p.adp && p.adp - pick >= 8
            ? ` <span class="valuetag">▼${Math.round(p.adp - pick)} vs ADP</span>`
            : '';
      const rookieRankStr = p.rookieRank ? ` #${p.rookieRank}` : '';
      const rookie = p.rookie
        ? '<span class="rookietag" title="Rookie Draft Rank' +
          rookieRankStr +
          '">R' +
          rookieRankStr +
          '</span>'
      ...
      const starBtn = !isTaken
        ? '<button type="button" class="watchbtn' +
          (watched ? ' active' : '') +
          '" title="' +
      ...
      if (!isTaken && scarcityData && scarcityData.playerAlerts) {
        ...
        if (scAlert) {
          scarcityTag =
            ' <span class="scarcity-tag' +
            (scAlert.isLast ? ' last-in-tier' : '') +
            '" title="' +
            scAlert.label +
            '">' +
            scAlert.label +
      ...
      if (isTaken) {
        ...
        const keeperObj = Array.isArray(global.state.keepers)
          ? global.state.keepers.find((k) => k && k.playerId === p.id)
          : null;
        if (keeperObj) {
          const isMine = keeperObj.slot === s.slot;
          const kTeamName = getTeamName(keeperObj.slot);
          actionCell =
            '<td><span class="picktag keeper-tag' +
            (isMine ? ' mine' : '') +
            '">' +
            '🔒 Keeper · ' +
            kTeamName +
          ...
          actionCell =
            ...
            tInfo.name +
            ' · ' +
            fmtPick(pickEntry.overall, s.teams) +
            ' (#' +
            pickEntry.overall +
            ')</span></td>';
        } else {
        ...
        if (isOurPick) {
          actionCell =
            '<td style="white-space:nowrap"><button class="act mine" title="Draft ' +
            p.name.replace(/"/g, '&quot;') +
            ' for our team" onclick="draftPlayer(' +
            p.id +
            ',true)">Select our Player</button></td>';
        } else {
          actionCell =
            '<td style="white-space:nowrap"><button class="act" title="Record pick for ' +
            onClock.name.replace(/"/g, '&quot;') +
            '" onclick="draftPlayer(' +
            p.id +
            ',false)">Pick Player</button></td>';
        }
      }
      ...
      if (p.bye) {
        ...
        if (byeClash.type === 'same-pos') {
          ...
          byeCell =
            '<td class="col-center"><span class="byetag bye-same-pos" title="' +
            tip.replace(/"/g, '&quot;') +
            '">⚠️ ' +
            p.bye +
            '</span></td>';
          ...
          byeCell =
            '<td class="col-center"><span class="byetag bye-other-pos" title="' +
            tip.replace(/"/g, '&quot;') +
            '">' +
            p.bye +
            '</span></td>';
      ...
      if (isRedraft) {
        ...
        const espnRank =
          (s.scoring === 'std' ? (p.espn_std ?? p.espn_ppr) : (p.espn_ppr ?? p.espn_std)) ??
          p.yahoo ??
          null;
      ...
      const dragAttrs = isWatchlistTab
        ? ' class="' +
          tierRow +
          (isTaken ? ' takenrow' : '') +
          ' watch-drag-row" draggable="true" title="Drag to reorder priority" ondragstart="onWatchDragStart(event, ' +
          p.id +
          ')" ondragover="onWatchDragOver(event)" ondragleave="onWatchDragLeave(event)" ondrop="onWatchDrop(event, ' +
          p.id +
          ')" ondragend="onWatchDragEnd(event)"'
        : ` class="${tierRow}${isTaken ? ' takenrow' : ''}"`;

      const rankCell = isWatchlistTab
        ? '<td class="rk watch-rk-cell"><span class="watch-drag-grip" title="Drag to reorder">⠿</span>' +
          (idx + 1) +
          tierBadgeHtml +
          '</td>'
    ...
    if (global.ui.posFilter === 'WATCHLIST' && global.PLAYERS.length) {
      emptyMsg =
        'Your watchlist is empty. Click the ☆ star on any player to add them to your watchlist.';
    }
    if ($('pool'))
      $('pool').innerHTML = html || `<tr><td colspan="11" class="empty">${emptyMsg}</td></tr>`;
  }
  ...
  function renderRosterSection(resolvedPicks, rosterSlots, teamsCount) {
    ...
    for (const { player } of resolvedPicks) {
      if (player.bye) {
        if (!byBye[player.bye]) byBye[player.bye] = [];
        byBye[player.bye].push(player.name);
      }
    }

    const formatSlot = (item, isStarter) =>
      formatRosterSlotHtml(item, isStarter, teamsCount, byBye);

    const filledStartersCount = allocation.starters.filter((s) => s.player !== null).length;
    let html =
      '<div class="roster-sec-title"><span>Starters</span><span>' +
      filledStartersCount +
      ' / ' +
      allocation.totalStarters +
      '</span></div>';
    html += allocation.starters.map((s) => formatSlot(s, true)).join('');

    html +=
      '<div class="roster-sec-title" style="margin-top:10px"><span>Bench</span><span>' +
      allocation.bench.length +
      ' / ' +
      allocation.totalBench +
      '</span></div>';
    if (allocation.bench.length > 0) {
      html += allocation.bench.map((b) => formatSlot(b, false)).join('');
    } else {
      html +=
        '<div class="meta" style="font-size:11.5px; padding:3px 0">No bench players yet.</div>';
    }
    ...
    const byeWarnHtml = clashWeeks.length
      ? '<div class="warnbox" style="margin-top:10px">Bye overlap — week ' +
        clashWeeks.map((w) => `${w}: ${byBye[w].join(', ')}`).join(' · week ') +
        '</div>'
      : '';
    ...
    return {
      ...
      needsHtml:
        needsList.join('') +
        '<span class="need">Lineup: ' +
        formatLineupSummary(rosterSlots) +
        '</span>',
  ...
  function getRosterPicksForSlot(targetSlot) {
    ...
    const loggedNames = new Set(
      teamPicks.map((e) => (e.customName ? normalizeName(e.customName) : '')).filter(Boolean),
    );
  ...
  function renderMyRoster() {
    ...
    const {
      ...
      draftedCount,
      keeperCount,
    } = getRosterPicksForSlot(targetSlot);

    if ($('myrosterheader')) {
      const kText = keeperCount > 0 ? ` · ${keeperCount} kept` : '';
      $('myrosterheader').innerHTML =
        '⭐ ' +
        myTeamName +
        ' <span class="meta" style="font-size:12px; margin-left:auto; font-weight:normal">Slot ' +
        targetSlot +
        ' · ' +
        draftedCount +
        ' drafted' +
  ...
  function renderInspectRoster() {
    const s = global.state.settings;
    const pick = currentPick();
    const onClockTeam = teamForOverall(
      pick,
      s.teams,
      s.mode,
      s.teamNames,
    ...
    const targetSlot =
      global.viewingRosterSlot != null ? global.viewingRosterSlot : onClockTeam.slot;
    const isFollowingClock =
      global.viewingRosterSlot == null || global.viewingRosterSlot === onClockTeam.slot;
    ...
    if ($('rosterTeamSelect')) {
      let selectHtml =
        '<option value="clock"' +
        (global.viewingRosterSlot == null ? ' selected' : '') +
        '>⚡ [Auto: On The Clock — ' +
        onClockTeam.name +
        ' (Slot ' +
        onClockTeam.slot +
    ...
    const kLabel =
      keeperCount > 0
        ? ` <span class="meta" style="font-size:11px">(${keeperCount} kept)</span>`
        : '';
    ...
    if ($('clockteamlabel')) {
      if (isFollowingClock) {
        $('clockteamlabel').innerHTML =
          '🕒 <b style="color:var(--accent)">On The Clock:</b> <span style="color:var(--text); font-weight:600">' +
          onClockTeam.name +
          '</span>' +
          (onClockTeam.isMe
            ? ' <span style="color:var(--good); font-weight:700">(Your Turn!)</span>'
            : ` (Slot ${onClockTeam.slot})`) +
          kLabel;
      } else {
        $('clockteamlabel').innerHTML =
          '👥 <b style="color:var(--dim)">Inspecting:</b> <span style="color:var(--text); font-weight:600">' +
          getTeamName(targetSlot) +
          '</span> (Slot ' +
          targetSlot +
          ' · ' +
          draftedCount +
          ' picks' +
          (keeperCount > 0 ? ` · ${keeperCount} kept` : '') +
          ') · <a href="javascript:void(0)" onclick="selectRosterSlot(null)" style="color:var(--accent); text-decoration:underline">Back to On-Clock</a>';
      }
    }
      ...
      global.state.log
        ...
        .map((e) => {
          ...
          const unlistedBadge = p.isUnlisted
            ? ' <span class="meta" style="font-size:10px">(unlisted)</span>'
            : '';
          const isKeeper = e.isKeeper || p.isKeeper;
          const keeperBadge = isKeeper
            ? ' <span class="keeper-badge" title="Keeper Selection" style="background:#3a2818; color:var(--warn); border:1px solid var(--warn); padding:1px 4px; border-radius:3px; font-size:10px; font-weight:600">🔒 Keeper</span>'
            : '';
          return (
            '<div class="logitem' +
            (isMe ? ' mine' : '') +
            ...
            e.overall +
            ' (' +
            fmtPick(e.overall, s.teams) +
            ')</span> ' +
            '<b style="color:' +
            (isMe ? 'var(--good)' : 'var(--text)') +
            '">' +
            tInfo.name +
            (isMe ? ' (You)' : '') +
            '</b>: ' +
  ...
  function renderTabs() {
    ...
    if ($('postabs')) {
      $('postabs').innerHTML = tabs
        .map(
          (t) =>
            '<span class="tab' +
            (global.ui.posFilter === t.id ? ' on' : '') +
            '" onclick="setFilter(\'' +
            t.id +
            '\')">' +
            t.label +
  ...
  function renderWatchlistPanel() {
    ...
    (global.state.watchlist || []).forEach((id, idx) => {
    ...
    if ($('mywatchlistcount')) {
      $('mywatchlistcount').textContent = scoredWatched.length
        ? `${scoredWatched.length} players`
        : 'Empty';
    }

    const myRoster = getMyRosterPlayers(
      global.state.log,
      byId,
    ...
    $('leftwatchlist').innerHTML =
      scoredWatched
        .map((p, idx) => {
          const posClass = order.includes(p.pos) ? p.pos : 'other';
          const byeClash = getByeClashStatus(p, myRoster);
          let byeHtml = p.bye
            ? `<span class="bye">bye ${p.bye}</span>`
            : '<span class="bye">—</span>';
          if (p.bye && byeClash.type === 'same-pos') {
            const names = byeClash.samePos.map((x) => x.name).join(', ');
            byeHtml =
              '<span class="bye clash" title="Same-position bye clash with ' +
              names +
              '">⚠️ ' +
              p.bye +
              '</span>';
            ...
            const names = byeClash.otherPos.map((x) => `${x.name} (${x.pos})`).join(', ');
            byeHtml =
              '<span class="bye" style="color:var(--warn); font-weight:600" title="Bye coincides with ' +
              names +
              '">' +
              p.bye +
              '</span>';
          ...
          const teamBadge =
            p.team && p.team !== '—' ? ` <span class="meta team-meta">${p.team}</span>` : '';
          const rookieRankStr = p.rookieRank ? ` #${p.rookieRank}` : '';
          const rookieBadge = p.rookie ? `<span class="rookietag">R${rookieRankStr}</span>` : '';

          return (
            '<div class="watchlist-item" draggable="true" title="Drag to reorder priority" ondragstart="onWatchDragStart(event, ' +
            p.id +
            ')" ondragover="onWatchDragOver(event)" ondragleave="onWatchDragLeave(event)" ondrop="onWatchDrop(event, ' +
            p.id +
            ')" ondragend="onWatchDragEnd(event)">' +
            '<span class="watch-rank-num">#' +
            (idx + 1) +
            '</span>' +
            '<button type="button" class="watchbtn active" title="Remove from Watchlist" onclick="toggleWatch(' +
            p.id +
            ', event)">★</button>' +
            '<span class="pos ' +
            posClass +
            ...
            p.pos +
            '</span>' +
            '<span class="pname" onclick="showPlayer(' +
            p.id +
        ...
        .join('') ||
      '<div class="meta" style="padding:10px 0">No players on watchlist.<br>Click the ☆ on any player to track them.</div>';
  }
  ...
  function renderBanner() {
      ...
      if (!d.players?.length) {
        ...
        try {
          const toastPending =
            ...
            sessionStorage.getItem('pendingDataRefreshToast');
          if (toastPending) {
            sessionStorage.removeItem('pendingDataRefreshToast');
            bannerHtml =
              '<div id="refresh_reconcile_toast" class="warnbox" style="background:#13241b; border:1px solid #16a34a; color:#86efac; margin-bottom:10px; display:flex; justify-content:space-between; align-items:center; padding:8px 12px; border-radius:6px;">' +
    ...
    if ($('sources')) {
      $('sources').textContent = d.players?.length
        ? d.players.length +
          ' players · data gathered ' +
          (d.generated || '') +
          ' · Mode: ' +
          qbStr +
          ' (' +
          scStr +
          ') · Blended rankings synced'
        : '';
    }
  }
  ...
  function render() {
    ...
    if (isBoardModalOpen && $('modalbox')?.classList.contains('modal-board')) {
  ...
  function closeModal() {
    if (
      typeof global.saveSleeperSyncSettings === 'function' &&
      document.getElementById('sync_sleeper_draft_id')
    ) {
      global.saveSleeperSyncSettings(false);
    }
    isBoardModalOpen = false;
    _returnToBoardOnClose = false;
    setupIsDirty = false;
  ...
  function scrollBoardToCurrentPick(smooth = true) {
    ...
    if (container && clockCell) {
      const targetLeft =
        clockCell.offsetLeft - container.clientWidth / 2 + clockCell.clientWidth / 2;
      const targetTop =
        clockCell.offsetTop - container.clientHeight / 2 + clockCell.clientHeight / 2;
      container.scrollTo({
  ...
  function renderDraftBoardModalView() {
    ...
    const totalPicks = s.teams * s.rounds;
    const isComplete = pick > totalPicks;
    const onClockTeam = !isComplete
      ? teamForOverall(pick, s.teams, s.mode, s.teamNames, s.slot, global.state.tradedPicks)
      : null;
    ...
    const tabsHtml =
      ...
      '<button type="button" class="board-tab-btn' +
      tabGridActive +
      '" onclick="setBoardActiveTab(\'grid\')">📊 Board Grid</button>' +
      '<button type="button" class="board-tab-btn' +
      tabStratActive +
      '" onclick="setBoardActiveTab(\'strategy\')">🎯 Live Strategy & Needs</button>' +
      '<button type="button" class="board-tab-btn' +
      tabSumActive +
      '" onclick="setBoardActiveTab(\'summary\')">🏆 League Value & Grades</button>' +
      '</div>';
    ...
    if (boardActiveTab === 'grid') {
      ...
      const filterButtonsHtml = filters
        .map((f) => {
          ...
          return (
            '<button type="button" class="board-filter-btn' +
            customClass +
            activeClass +
      ...
      const densityGroupHtml =
        '<div class="board-density-toggle" role="group" aria-label="Board density">' +
        '<button type="button" class="board-density-btn' +
        (boardDensity === 'normal' ? ' active' : '') +
        '" onclick="setBoardDensity(\'normal\')" title="Normal Density View">👁️ Normal</button>' +
        '<button type="button" class="board-density-btn' +
        (boardDensity === 'compact' ? ' active' : '') +
        '" onclick="setBoardDensity(\'compact\')" title="Compact Density View">🗜️ Compact</button>' +
        '</div>';
      ...
      toolbarActionsHtml =
        ...
        (!isComplete
          ? '<button type="button" class="act" onclick="scrollBoardToCurrentPick(true)" style="font-size:11.5px; padding:3px 8px; font-weight:600; color:var(--accent); border-color:var(--accent)">⚡ Jump to On-Clock</button>'
          : '') +
        densityGroupHtml +
      ...
      toolbarActionsHtml =
        '<div class="board-actions">' +
        (!isComplete
          ? '<button type="button" class="act" onclick="setBoardActiveTab(\'grid\'); setTimeout(() => scrollBoardToCurrentPick(true), 60)" style="font-size:11.5px; padding:3px 8px; font-weight:600; color:var(--accent); border-color:var(--accent)">⚡ Jump to Grid On-Clock</button>'
          : '') +
        '<button type="button" class="act primary" onclick="closeBoardModal()" style="font-size:11.5px; padding:3px 10px; font-weight:700">Done</button>' +
        '</div>';
    } else {
    ...
    if (boardActiveTab === 'grid') {
      const boardData =
        typeof generateDraftBoardGrid === 'function'
      ...
      for (let slot = 1; slot <= s.teams; slot++) {
        ...
        const name =
          (Array.isArray(s.teamNames) ? s.teamNames[slot - 1] : null) ||
          (isMe ? 'My Team' : `Team ${slot}`);
        const isClock = onClockTeam && onClockTeam.slot === slot;
        const thClasses = ['board-th', isMe ? 'is-me' : '', isClock ? 'is-clock' : '']
          .filter(Boolean)
          .join(' ');
        ...
        tableHeadHtml +=
          ...
          thClasses +
          '">' +
          '<div class="board-th-team" title="' +
          name +
          '">' +
          (isMe ? '⭐ ' : '') +
          name +
          '</div>' +
          '<div class="board-th-slot">SLOT ' +
          slot +
          (isMe ? ' <span style="font-size:9px; color:var(--good)">(YOU)</span>' : '') +
          '</div>' +
          '</th>';
      }
      ...
      for (let rIdx = 0; rIdx < boardData.grid.length; rIdx++) {
        ...
        tableBodyHtml += '<tr>';
        tableBodyHtml +=
          '<th class="board-round-th">' +
          '<div class="board-round-num">R' +
          rNum +
          '</div>' +
          '<div class="board-round-arrow" title="' +
          (row.isForward ? 'Forward (1 to N)' : 'Reverse (N to 1)') +
          '">' +
          arrow +
          '</div>' +
          '</th>';
        ...
        for (let cIdx = 0; cIdx < row.picks.length; cIdx++) {
          ...
          if (pickCell.isDrafted && pickCell.player) {
            ...
            const posClass = ['QB', 'RB', 'WR', 'TE', 'K'].includes(posUpper)
              ...
              : ['DST', 'DEF', 'D/ST'].includes(posUpper)
                ? 'dst'
                : 'other';
            ...
            const unlistedBadge = p.isUnlisted
              ? ' <span style="font-size:10px; font-weight:normal; opacity:0.8">(custom)</span>'
              : '';
            const keeperBadge = isKeeper
              ? '<span class="keeper-tag-chip" title="Keeper Selection">🔒</span>'
              : '';
            const tradedChip = isTraded
              ? '<span class="traded-tag" title="Originally ' +
                pickCell.originalTeamName +
                '">via ' +
                pickCell.originalTeamName +
                '</span>'
              : '';
            ...
            if (isCompact) {
              ...
              if (isKeeper) {
                topEndHtml =
                  ...
                  (teamByeStr || '—') +
                  '</span>' +
                  '<span class="board-card-pos">' +
                  (p.pos || '—') +
                  '</span>';
              } else {
                topEndHtml =
                  (tradedChip ? `${tradedChip} ` : '') +
                  '<span class="board-card-compact-meta">' +
                  (teamByeStr || p.pos || '—') +
                  '</span>';
              }
              ...
              tableBodyHtml +=
                ...
                '<span class="board-card-pick">#' +
                overall +
                ' (' +
                fmtPick(overall, s.teams) +
                ')</span>' +
                '<div class="board-card-top-end">' +
                topEndHtml +
                '</div>' +
                '</div>' +
                '<div class="board-card-name" title="' +
                (p.name || '').replace(/"/g, '&quot;') +
                '">' +
                (p.name || 'Unlisted') +
                unlistedBadge +
            ...
            if (isCompact) {
              tableBodyHtml +=
                ...
                overall +
                '">' +
                '<div class="board-card pending-keeper ' +
                posClass +
                '" title="Keeper Assignment (Round ' +
                pickCell.round +
                ')">' +
                '<div class="board-card-top">' +
                '<span class="board-card-pick">#' +
                ...
                '<div class="board-card-name" title="' +
                (p.name || '').replace(/"/g, '&quot;') +
                '"><span class="keeper-tag-chip" style="margin-right:3px">🔒</span>' +
                p.name +
                '</div>' +
                '</div>' +
                '</td>';
              ...
              tableBodyHtml +=
                ...
                fmtPick(overall, s.teams) +
                ')</span>' +
                '<span class="board-card-team" style="font-size:10.5px">' +
                (teamByeStr || '—') +
                '</span>' +
                '</div>' +
                '<div class="board-card-name" title="' +
            ...
            const tradedChip = pickCell.isTraded
              ? `<span class="traded-tag">via ${pickCell.originalTeamName}</span>`
              : '';
            ...
            tableBodyHtml +=
              ...
              '<span class="board-empty-sub">Slot ' +
              pickCell.effectiveSlot +
              '</span>' +
              tradedChip +
              '</div>' +
              '</div>' +
      ...
      try {
        const taken =
          typeof takenMap === 'function'
            ? takenMap()
            : typeof global.takenMap === 'function'
              ? global.takenMap()
              : new Map();
        const allScored =
          typeof scored === 'function'
            ? scored()
            : Array.isArray(global.PLAYERS)
              ? global.PLAYERS
              : [];
        const availablePlayers = allScored.filter((p) => p && p.id != null && !taken.has(p.id));

        const strat =
          typeof analyzeLiveDraftStrategy === 'function'
        ...
        if (!strat) {
          ...
          if (strat.isOnClock) {
            ...
            if (modalScarcity && strat.recommendedTargets && strat.recommendedTargets.length > 0) {
              const topScarcityTarget =
                strat.recommendedTargets.find((t) => {
                  const alert = modalScarcity.playerAlerts
                    ? modalScarcity.playerAlerts.get(t.id)
                    : null;
                  return alert?.isLast;
              ...
              if (topScarcityTarget) {
                ...
                cliffAlertHtml =
                  '<div style="font-size:12px; color:#fbbf24; margin-top:4px; display:flex; align-items:center; gap:4px">⚠️ <strong>Tier Cliff Alert:</strong> ' +
                  topScarcityTarget.name +
                  ' is ' +
                  (a.isLast ? 'the last available' : `one of only ${a.remaining} remaining`) +
                  ' Tier ' +
                  a.tier +
                  ' ' +
                  topScarcityTarget.pos +
                  '!</div>';
              }
            }

            bannerHtml =
              '<div class="strategy-banner on-clock">' +
              '<div>' +
              '<h4 style="margin:0; font-size:16px; color:var(--good); display:flex; align-items:center; gap:6px">⚡ YOU ARE ON THE CLOCK! (Pick #' +
              pick +
              ')</h4>' +
            ...
            bannerHtml =
              ...
              '<div style="font-size:12px; color:#cbd5e1">All ' +
              totalPicks +
              ' picks have been drafted. Switch to the League Value & Grades tab to view full team rankings and superlatives.</div>' +
              '</div>' +
            ...
            bannerHtml =
              '<div class="strategy-banner">' +
              '<div>' +
              '<h4 style="margin:0; font-size:15px; color:var(--text)">⏳ ' +
              strat.picksUntilUserTurn +
              ' Pick' +
              (strat.picksUntilUserTurn === 1 ? '' : 's') +
              ' Until Your Turn</h4>' +
              '<div style="font-size:12px; color:var(--dim); margin-top:2px">Your next turn is <strong style="color:var(--accent)">Pick #' +
              strat.nextUserPick +
              ' (' +
              fmtPick(strat.nextUserPick, s.teams) +
              ')</strong> · Currently on clock: <strong style="color:var(--text)">' +
              (onClockTeam ? onClockTeam.name : `Pick #${pick}`) +
              '</strong></div>' +
              '</div>' +
              '</div>';
          } else {
          ...
          if (strat.opponentThreats && strat.opponentThreats.length > 0) {
            const cardsHtml = strat.opponentThreats
              .map((t) => {
                const needsHtml =
                  t.urgentNeeds
                    .map((n) => {
                      ...
                      const posBadgeClass =
                        posUpper === 'DST' || posUpper === 'DEF' || posUpper === 'D/ST'
                          ? 'DST'
                          : posUpper;
                      return (
                        '<span class="pos ' +
                        posBadgeClass +
                        '" style="font-size:9.5px; font-weight:800; padding:1px 5px; border-radius:3px">' +
                        n.pos +
                        '</span>'
                      );
                    })
                    .join('') ||
                  '<span style="font-size:10px; color:var(--dim)">Depth / Bench</span>';
                ...
                return (
                  ...
                  t.pickFmt +
                  ' <span style="font-size:9.5px; font-weight:normal; color:var(--dim)">(#' +
                  t.overall +
                  ')</span></span>' +
                  '<span style="font-size:10px; color:var(--dim)">Slot ' +
                  t.slot +
                  '</span>' +
                  '</div>' +
                  '<div class="threat-card-team">' +
                  t.teamName +
                  '</div>' +
                  '<div style="font-size:10px; color:var(--dim); margin-top:2px">Target Needs:</div>' +
                  '<div class="threat-needs-list">' +
                  needsHtml +
                  '</div>' +
                  '</div>'
                );
            ...
            const dangersHtml = (strat.runDangers || [])
              .map((d) => {
            ...
            const threatTitle = strat.isOnClock
              ? '🛡️ Anticipated Opponents Ahead (' +
                strat.opponentThreats.length +
                ' Picks Until Pick #' +
                strat.subsequentUserPick +
                ')'
              : '🛡️ Opponents Drafting Before Your Turn (' +
                strat.opponentThreats.length +
                ' Picks Ahead)';
            ...
            threatsHtml =
              '<div class="threat-timeline-wrapper">' +
              ...
              '<div class="strategy-section-title" style="margin-bottom:0">' +
              threatTitle +
              '</div>' +
              '<button type="button" class="act" style="font-size:11px; padding:2px 8px" onclick="event.stopPropagation(); toggleStrategyThreats()">' +
              collapseBtnLabel +
              '</button>' +
              '</div>' +
              (!strategyThreatsCollapsed
                ? '<div class="threat-timeline" style="margin-top:10px">' +
                  cardsHtml +
                  '</div>' +
            ...
            strat.isOnClock &&
            strat.subsequentUserPick != null &&
            strat.subsequentUserPick === pick + 1
          ) {
            threatsHtml =
              '<div class="threat-timeline-wrapper" style="padding:10px 14px">' +
              '<div style="font-size:12.5px; font-weight:700; color:var(--good); display:flex; align-items:center; gap:6px">' +
              '⚡ Back-to-Back Picks! You also have the next pick: #' +
              strat.subsequentUserPick +
              ' (' +
              fmtPick(strat.subsequentUserPick, s.teams) +
              '). No opponents draft between your turns.' +
              '</div>' +
              '</div>';
          }
          ...
          const needsCardsHtml = (strat.userNeeds || [])
            .map((n) => {
              ...
              return (
                ...
                ' / ' +
                n.baseReq +
                ' <span style="font-size:11px; font-weight:normal; color:var(--dim)">starters</span></div>' +
                '<div style="background:rgba(255,255,255,0.08); height:4px; border-radius:2px; overflow:hidden"><div style="background:' +
                barColor +
                '; width:' +
                barPct +
                '%; height:100%"></div></div>' +
          ...
          if (strat.targetsByPosition) {
            ...
            const visiblePositions = posOrder.filter((pos) => {
              if (pos === 'K' || pos === 'DST') {
                return (
                  isLateRounds ||
                  strat.userNeeds.some(
                    (n) => n.pos === pos && (n.urgency === 'CRITICAL' || n.urgency === 'NEEDED'),
                  )
                );
              }
              return true;
            ...
            const posColumnsHtml = visiblePositions
              .map((pos) => {
                const pList = strat.targetsByPosition[pos] || [];
                const needInfo = strat.userNeeds.find((n) => n.pos === pos) || {
                  urgency: 'FILLED',
                  label: 'Filled',
                };
                ...
                if (pList.length === 0) {
                  rowsHtml =
                    '<div style="font-size:11px; color:var(--dim); padding:10px 0; text-align:center">No available players</div>';
                } else {
                  rowsHtml = pList
                    .map((p, idx) => {
                      const isW = Boolean(p.isWatched || global.state.watchlist?.includes(p.id));
                      const starIcon = isW ? '★' : '☆';
                      ...
                      if (p.bye) {
                        ...
                        if (bClash.type === 'same-pos') {
                          ...
                          const tip =
                            'Same-position bye clash with ' +
                            (names || 'roster') +
                            ' (Week ' +
                            p.bye +
                            ')';
                          byeHtml =
                            '<span class="target-bye-pill clash" title="' +
                            tip.replace(/"/g, '&quot;') +
                            '">⚠️ Wk ' +
                            p.bye +
                            '</span>';
                          ...
                          byeHtml =
                            '<span class="target-bye-pill overlap" title="' +
                            tip.replace(/"/g, '&quot;') +
                            '">⚡ Wk ' +
                            p.bye +
                            '</span>';
                      ...
                      const surplusTag =
                        p.valSurplus > 0
                          ? '<span style="color:var(--good); font-size:10px; font-weight:700">+' +
                            p.valSurplus +
                            ' vs ADP</span>'
                          : p.adp
                            ? '<span style="color:var(--dim); font-size:10px">ADP ' +
                              p.adp +
                              '</span>'
                            : '';

                      const pAlert = modalScarcity?.playerAlerts
                        ? modalScarcity.playerAlerts.get(p.id)
                        : null;
                      const scarcityTag = pAlert
                        ? ' <span class="scarcity-tag' +
                          (pAlert.isLast ? ' last-in-tier' : '') +
                          '" style="font-size:8.5px; padding:0 4px">' +
                          (pAlert.isLast
                            ? `⚡ Last in T${pAlert.tier}`
                            : `⚠️ 2 in T${pAlert.tier}`) +
                          '</span>'
                        : '';
                      ...
                      return (
                        '<div class="target-row" onclick="showPlayer(' +
                        p.id +
                        ')">' +
                        ...
                        '" title="' +
                        starTitle +
                        '" onclick="event.stopPropagation(); toggleWatch(' +
                        p.id +
                        '); renderDraftBoardModalView();">' +
                        starIcon +
                        '</button>' +
                        '<span class="target-rank-num">#' +
                        (idx + 1) +
                        '</span>' +
                        ...
                        p.score +
                        ' <span style="font-size:9.5px; font-weight:normal; color:var(--dim)">pts</span></div>' +
                        '<div>' +
                        surplusTag +
                        '</div>' +
                        '</div>' +
          ...
          const scoredWatched = allScored
            .filter(
              (p) =>
                p && p.id != null && isWatched(global.state.watchlist, p.id) && !taken.has(p.id),
            )
            .sort((a, b) => (watchedOrder.get(a.id) ?? 999) - (watchedOrder.get(b.id) ?? 999));
          ...
          if (scoredWatched.length === 0) {
            watchlistCardsHtml =
              '<div class="strategy-empty-watchlist">' +
            ...
            watchlistCardsHtml =
              '<div class="strategy-watchlist-grid">' +
              scoredWatched
                .map((p, idx) => {
                  const posUpper = (p.pos || '').toUpperCase();
                  const posClass = ['QB', 'RB', 'WR', 'TE', 'K'].includes(posUpper)
                  ...
                  const surplus =
                    p.adp != null && strat.nextUserPick != null
                      ? Math.round(strat.nextUserPick - p.adp)
                      : 0;
                  const surplusHtml =
                    surplus > 0
                      ? '<span style="color:var(--good); font-weight:700">+' +
                        surplus +
                        ' vs ADP</span>'
                      : p.adp
                        ? `<span>ADP ${p.adp}</span>`
                  ...
                  const scarcityTag = pAlert
                    ? ' <span class="scarcity-tag' +
                      (pAlert.isLast ? ' last-in-tier' : '') +
                      '" style="font-size:8.5px; padding:0 4px">' +
                      (pAlert.isLast ? `⚡ Last in T${pAlert.tier}` : `⚠️ 2 in T${pAlert.tier}`) +
                      '</span>'
                    : '';
                  const tierBadge = p.posTier ? ` ${formatTierPill(p.posTier)}` : '';
                  ...
                  return (
                    '<div class="watchlist-item strategy-watch-card" draggable="true" title="Drag to reorder priority" ondragstart="onWatchDragStart(event, ' +
                    p.id +
                    ')" ondragover="onWatchDragOver(event)" ondragleave="onWatchDragLeave(event)" ondrop="onWatchDrop(event, ' +
                    p.id +
                    ')" ondragend="onWatchDragEnd(event)">' +
                    ...
                    (idx + 1) +
                    '</span>' +
                    '<button type="button" class="target-star-btn active" title="Remove from Watchlist" onclick="event.stopPropagation(); toggleWatch(' +
                    p.id +
                    ', event);">★</button>' +
                    '<span class="pos ' +
                    posClass +
                    ...
                    p.pos +
                    '</span>' +
                    '<div style="cursor:pointer; flex:1; min-width:0; overflow:hidden" onclick="showPlayer(' +
                    p.id +
                    ')">' +
                    '<div style="display:flex; align-items:center; gap:5px; white-space:nowrap; overflow:hidden; text-overflow:ellipsis">' +
                    '<span style="font-size:11.5px; font-weight:700; color:var(--text); white-space:nowrap; overflow:hidden; text-overflow:ellipsis">' +
                    p.name +
                    '</span>' +
                    tierBadge +
                    scarcityTag +
                    ...
                    '<div style="text-align:right; flex-shrink:0">' +
                    '<div style="font-size:12px; font-weight:800; color:var(--accent)">' +
                    (p.score != null ? Math.round(p.score * 10) / 10 : '—') +
                    ' <span style="font-size:9.5px; font-weight:normal; color:var(--dim)">pts</span></div>' +
                    '</div>' +
                    '</div>'
                  );
          ...
          const watchlistHtml =
            '<div class="strategy-watchlist-section">' +
            ...
            '<span>⭐ Your Watchlist (' +
            scoredWatched.length +
            ' Available Players · Drag to Reorder)</span>' +
        ...
        mainViewHtml =
          '<div class="strategy-container"><div class="strategy-banner"><div><h4 style="margin:0; font-size:15px; color:var(--warn)">⚠️ Could not generate live strategy view (' +
          (err.message || 'unknown error') +
          ').</h4><div style="font-size:12px; color:var(--dim); margin-top:4px">You can switch back to the Board Grid or League Value tabs.</div></div><button type="button" class="act primary" onclick="setBoardActiveTab(\'grid\')">Return to Board Grid</button></div></div>';
      }
    } else {
      // Summary View
      const summary =
        typeof generateDraftSummaryAnalysis === 'function'
      ...
      const myGradeLetter = myT?.grade ? myT.grade.charAt(0).toLowerCase() : 'b';

      const myReportHtml = myT
        ? '<div class="summary-report-card">' +
          '<div class="summary-stat-box">' +
          '<div class="summary-stat-label">Your Grade</div>' +
          '<div class="summary-stat-val"><span class="grade-badge ' +
          myGradeLetter +
          '">' +
          myT.grade +
          '</span></div>' +
          ...
          myT.rank +
          ' <span style="font-size:12px; font-weight:normal; color:var(--dim)">of ' +
          s.teams +
          '</span></div>' +
          '</div>' +
          '<div class="summary-stat-box">' +
          '<div class="summary-stat-label">Total Value Score</div>' +
          '<div class="summary-stat-val" style="color:var(--accent)">' +
          myT.totalScore +
          ' <span style="font-size:11px; font-weight:normal; color:var(--dim)">pts</span></div>' +
          '</div>' +
          '<div class="summary-stat-box">' +
          ...
          '<div class="summary-stat-val" style="color:' +
          (myT.netAdpSurplus >= 0 ? 'var(--good)' : 'var(--warn)') +
          '">' +
          (myT.netAdpSurplus >= 0 ? '+' : '') +
          myT.netAdpSurplus +
          '</div>' +
          ...
          '<div class="summary-stat-val" style="font-size:12px; font-weight:700">' +
          (myT.bestSteal
            ? `${myT.bestSteal.player.name} (+${Math.round(myT.bestSteal.surplus)})`
            : '—') +
          '</div>' +
          '</div>' +
          '</div>'
      ...
      const superlativesHtml =
        '<div class="superlatives-grid">' +
        '<div class="superlative-card"><div class="superlative-title">👑 Top Rated Team</div><div class="superlative-winner">' +
        (sup.champion ? sup.champion.teamName : '—') +
        '</div><div class="superlative-detail">' +
        (sup.champion ? `${sup.champion.totalScore} total points (${sup.champion.grade})` : '') +
        '</div></div>' +
        '<div class="superlative-card"><div class="superlative-title">💎 Steal of the Draft</div><div class="superlative-winner">' +
        (sup.bestSteal ? sup.bestSteal.player.name : '—') +
        '</div><div class="superlative-detail">' +
        (sup.bestSteal
          ? '+' +
            Math.round(sup.bestSteal.surplus) +
            ' picks past ADP (' +
            (sup.bestSteal.player.effectiveTeamName || '') +
            ')'
          : '') +
        '</div></div>' +
        '<div class="superlative-card"><div class="superlative-title">🚨 Biggest Reach</div><div class="superlative-winner">' +
        (sup.biggestReach ? sup.biggestReach.player.name : '—') +
        '</div><div class="superlative-detail">' +
        (sup.biggestReach
          ? Math.round(sup.biggestReach.surplus) +
            ' picks ahead of ADP (' +
            (sup.biggestReach.player.effectiveTeamName || '') +
            ')'
          : '') +
        '</div></div>' +
        '<div class="superlative-card"><div class="superlative-title">🥇 Best QB Room</div><div class="superlative-winner">' +
        (sup.bestQb ? sup.bestQb.teamName : '—') +
        '</div><div class="superlative-detail">' +
        (sup.bestQb ? `${sup.bestQb.qbScore} QB points` : '') +
        '</div></div>' +
        '<div class="superlative-card"><div class="superlative-title">🥇 Best RB Room</div><div class="superlative-winner">' +
        (sup.bestRb ? sup.bestRb.teamName : '—') +
        '</div><div class="superlative-detail">' +
        (sup.bestRb ? `${sup.bestRb.rbScore} RB points` : '') +
        '</div></div>' +
        '<div class="superlative-card"><div class="superlative-title">🥇 Best WR Room</div><div class="superlative-winner">' +
        (sup.bestWr ? sup.bestWr.teamName : '—') +
        '</div><div class="superlative-detail">' +
        (sup.bestWr ? `${sup.bestWr.wrScore} WR points` : '') +
        '</div></div>' +
        '<div class="superlative-card"><div class="superlative-title">🥇 Best TE Room</div><div class="superlative-winner">' +
        (sup.bestTe ? sup.bestTe.teamName : '—') +
        '</div><div class="superlative-detail">' +
        (sup.bestTe ? `${sup.bestTe.teScore} TE points` : '') +
        '</div></div>' +
        '</div>';
      ...
      const tableRowsHtml = sortedTeams
        .map((t) => {
          ...
          let drawerHtml = '';
          if (isExpanded) {
            const startersListHtml = t.starters
              .map((st) => {
                const p = st.player;
                if (!p)
                  return (
            ...
            drawerHtml =
              ...
              '<div><strong style="font-size:11px; color:var(--text); text-transform:uppercase">Starters (' +
              t.startersScore +
              ' pts)</strong><div style="display:flex; flex-direction:column; gap:3px; margin-top:4px">' +
              startersListHtml +
              '</div></div>' +
              '<div><strong style="font-size:11px; color:var(--text); text-transform:uppercase">Bench (' +
              t.benchScore +
              ' pts)</strong><div style="display:flex; flex-direction:column; gap:3px; margin-top:4px">' +
              (benchListHtml ||
                '<span style="font-size:11px; color:var(--dim)">No bench players</span>') +
              '</div></div>' +
              '</div>' +
              '</td></tr>';
          }
          ...
          return (
            '<tr class="' +
            (t.isMe ? 'is-me' : '') +
            '" onclick="toggleTeamRosterDrawer(' +
            t.slot +
            ')" style="cursor:pointer" title="Click to view full roster">' +
            '<td class="summary-rank-cell">#' +
            t.rank +
            '</td>' +
            '<td><strong style="color:var(--text)">' +
            (t.isMe ? '⭐ ' : '') +
            t.teamName +
            '</strong> <span style="font-size:10px; color:var(--dim)">Slot ' +
            t.slot +
            '</span></td>' +
            ...
            t.benchScore +
            '</td>' +
            '<td><div class="summary-pos-cell"><span>' +
            t.qbScore +
            '</span><span class="pos-rank-pill' +
            (t.qbRank === 1 ? ' top' : '') +
            '">#' +
            t.qbRank +
            '</span></div></td>' +
            ...
            '</span></div></td>' +
            '<td style="color:' +
            (t.netAdpSurplus >= 0 ? 'var(--good)' : 'var(--warn)') +
            '">' +
            (t.netAdpSurplus >= 0 ? '+' : '') +
            t.netAdpSurplus +
            '</td>' +
      ...
      mainViewHtml =
        ...
        '<thead>' +
        '<tr>' +
        '<th onclick="setSummarySort(\'rank\')">Rank' +
        sortIndicator('rank') +
        '</th>' +
        '<th onclick="setSummarySort(\'slot\')">Team' +
        sortIndicator('slot') +
        '</th>' +
        '<th>Grade</th>' +
        '<th onclick="setSummarySort(\'totalScore\')">Total Score' +
        sortIndicator('totalScore') +
        '</th>' +
        '<th onclick="setSummarySort(\'startersScore\')">Starters / Bench' +
        sortIndicator('startersScore') +
        '</th>' +
        '<th onclick="setSummarySort(\'qbScore\')">QB Value' +
        sortIndicator('qbScore') +
        '</th>' +
        '<th onclick="setSummarySort(\'rbScore\')">RB Value' +
        sortIndicator('rbScore') +
        '</th>' +
        '<th onclick="setSummarySort(\'wrScore\')">WR Value' +
        sortIndicator('wrScore') +
        '</th>' +
        '<th onclick="setSummarySort(\'teScore\')">TE Value' +
        sortIndicator('teScore') +
        '</th>' +
        '<th onclick="setSummarySort(\'netAdpSurplus\')">ADP Value' +
        sortIndicator('netAdpSurplus') +
        '</th>' +
        '</tr>' +
        '</thead>' +
        '<tbody>' +
    ...
    $('modalbox').innerHTML =
      ...
      '<h3>📊 ' +
      (s.leagueName || 'Your Draft Board') +
      ' <span class="meta" style="font-size:13px; font-weight:normal">(' +
      s.teams +
      ' Teams · ' +
      s.rounds +
      ' Rounds · ' +
      (s.mode === '3rr' ? '3RR' : 'Snake') +
      ')</span></h3>' +
  ...
  function renderInjurySection(p) {
    const refreshDate =
      (window.DRAFT_DATA && (window.DRAFT_DATA.injuriesUpdated || window.DRAFT_DATA.generated)) ||
      'latest update';
    if (!p.injury?.code) {
      return (
        '<div class="injury-accordion inactive">' +
        '<div class="injury-header-inactive">' +
        '<span class="injury-inactive-icon">🩺</span>' +
        '<span class="injury-inactive-text">No injury reported as of ' +
        refreshDate +
        '</span>' +
        '</div>' +
        '</div>'
    ...
    const returnText = inj.returnDate
      ? `<span class="inj-returndate">Est. Return: <b>${inj.returnDate}</b></span>`
      : '';
    const partText = inj.type
      ? inj.type + (inj.detail ? ` (${inj.detail})` : '')
      : inj.detail || 'Undisclosed';
    const comments = inj.longComment || inj.shortComment || 'No additional notes reported.';

    return (
      '<div class="injury-accordion active inj-theme-' +
      c +
      '">' +
      ...
      returnText +
      '</div>' +
      '<span id="inj_accordion_arrow" class="dc-arrow">' +
      (injuryAccordionExpanded ? '▲' : '▼') +
      '</span>' +
      '</div>' +
      '<div id="inj_accordion_body" class="injury-body' +
      (injuryAccordionExpanded ? ' show' : '') +
      '">' +
      '<div class="injury-comment">' +
      comments +
      '</div>' +
      '<div class="injury-footer">Report Date: ' +
      (inj.date ? inj.date.split('T')[0] : refreshDate) +
      ' · Source: ESPN NFL Injury Report</div>' +
      '</div>' +
      '</div>'
    );
  }
  ...
  function renderDepthChart(p, taken) {
    if (!p.team || p.team === 'FA') {
      return (
        ...
        '<span id="dc_accordion_arrow" class="dc-arrow">' +
        (depthChartExpanded ? '▲' : '▼') +
        '</span>' +
        '</div>' +
        '<div id="dc_accordion_body" class="dc-body' +
        (depthChartExpanded ? ' show' : '') +
        '">' +
    ...
    const teamDc = window.DRAFT_DATA?.depthCharts ? window.DRAFT_DATA.depthCharts[p.team] : null;
    if (!teamDc) {
      return (
        '<div class="dc-accordion">' +
        '<div class="dc-header" onclick="toggleDepthChart()">' +
        ...
        (depthChartExpanded ? ' show' : '') +
        '">' +
        '<div class="meta" style="font-size:12px">Depth chart data not available for ' +
        p.team +
        '.</div>' +
        '</div></div>'
      );
    ...
    const renderRow = (label, badgeClass, athletes) => {
      ...
      return (
        '<div class="dc-pos-row">' +
        '<span class="dc-pos-badge ' +
        badgeClass +
        '">' +
        label +
        '</span>' +
        '<div class="dc-athletes-list">' +
        athletes.map(renderAthletePill).join('') +
        '</div>' +
        '</div>'
      );
    };
    ...
    if (teamDc.wr) {
      if (teamDc.wr.wr1?.length) rowsHtml += renderRow('WR1', 'wr', teamDc.wr.wr1);
      if (teamDc.wr.wr2?.length) rowsHtml += renderRow('WR2', 'wr', teamDc.wr.wr2);
      if (teamDc.wr.wr3?.length) rowsHtml += renderRow('WR3', 'wr', teamDc.wr.wr3);
    }
  ...
  function showPlayer(id) {
    ...
    if (!p) return;
    const s = global.state.settings;
    const _activeDyn = getDynastyRank(p, s.qbFormat);
    const _activeRed = getRedraftRank(p, s.qbFormat, s.scoring);
    ...
    if (taken.has(id)) {
      ...
      const keeperObj = Array.isArray(global.state.keepers)
        ? global.state.keepers.find((k) => k && k.playerId === id)
        : null;
      if (keeperObj) {
        const isMine = keeperObj.slot === s.slot;
        const kTeamName = getTeamName(keeperObj.slot);
        status =
          '<span class="picktag keeper-tag' +
          (isMine ? ' mine' : '') +
          '">🔒 Keeper · ' +
          kTeamName +
          ' (Round ' +
    ...
    const rawStats =
      typeof formatPlayerStats === 'function' ? formatPlayerStats(p, s, activeScore) : [];

    const stats = rawStats.map((x) => `<span class="stat">${x[0]}<b>${x[1]}</b></span>`).join('');

    const snapDate = window.DRAFT_DATA.extrasGenerated || window.DRAFT_DATA.generated || '';
    const blurb = p.blurb
      ? '<div class="blurb">' +
        p.blurb +
        '</div><div class="blurbnote">News snapshot from ' +
        snapDate +
        ' — use the links below for anything newer.</div>'
    ...
    const schedHtml = sched
      ? '<h4>2026 Schedule</h4><div class="schedgrid">' +
        sched
          .map(
            (opp, i) =>
              '<span class="g' +
        ...
        '</div>'
      : p.team && p.team !== 'FA'
        ? '<div class="blurbnote">Schedule data still loading — check back shortly.</div>'
        : '';
    const q = encodeURIComponent(
      `${p.name + (p.team && p.team !== 'FA' ? ` ${p.team}` : '')} fantasy`,
    );
    const links =
      '<div class="links">' +
      '<a target="_blank" href="https://news.google.com/search?q=' +
      q +
      '">🔎 Google News</a>' +
      '<a target="_blank" href="https://www.espn.com/search/_/q/' +
      encodeURIComponent(p.name) +
      '">ESPN</a>' +
      '<a target="_blank" href="https://www.google.com/search?q=' +
      encodeURIComponent(p.name) +
      '+site%3Afantasypros.com">FantasyPros</a>' +
      '</div>';
    ...
    if (byeClash.type === 'same-pos') {
      const names = byeClash.samePos.map((x) => x.name).join(', ');
      byeAlert =
        '<div class="warnbox" style="margin-top:10px; background:#3d2026; border-color:var(--bad); color:var(--bad)">⚠️ <b>Same-Position Bye Clash:</b> Shares Week ' +
        p.bye +
        ' bye with your ' +
        p.pos +
        ' (' +
      ...
      const names = byeClash.otherPos.map((x) => `${x.name} (${x.pos})`).join(', ');
      byeAlert =
        '<div style="margin-top:10px; padding:8px 10px; background:#33261a; border:1px solid var(--warn); color:var(--warn); border-radius:8px; font-size:12.5px">⚡ <b>Roster Bye Overlap:</b> Shares Week ' +
        p.bye +
        ' bye with ' +
        names +
        '</div>';
  ...
  function showUnlistedPlayer(overall) {
    ...
    if (pBox) {
      ...
      pBox.innerHTML =
        ...
        '<button class="close" onclick="closePlayerModal()">×</button></h3>' +
        '<div class="statrow">' +
        '<span class="stat">Drafted By<b>' +
        tInfo.name +
        (isMine ? ' (You)' : '') +
        '</b></span>' +
        '<span class="stat">Pick<b>#' +
        entry.overall +
        ' (' +
        fmtPick(entry.overall, global.state.settings.teams) +
        ')</b></span>' +
  ...
  function openUnlistedPickModal() {
    ...
    const posButtons = positions
      .map(
        (pos) =>
          '<button type="button" class="tab' +
          (pos === unlistedSelectedPos ? ' on' : '') +
          '" id="unlisted_pos_' +
          pos +
          '" onclick="selectUnlistedPos(\'' +
          pos +
          '\')">' +
          pos +
          '</button>',
    ...
    $('modalbox').innerHTML =
      ...
      'Drafting for: <b style="color: ' +
      (who.isMe ? 'var(--good)' : 'var(--text)') +
      '; font-size: 14px">' +
      who.name +
      (who.isMe ? ' (Your Team)' : '') +
      '</b> ' +
      ...
      pick +
      ' (' +
      fmtPick(pick, global.state.settings.teams) +
      ')</b>' +
      '</div>' +
      ...
      '<div class="tabs" style="flex-wrap:wrap; gap:6px" id="unlisted_pos_tabs">' +
      posButtons +
      '</div>' +
      '</div>' +
      '<div class="setup-grid" style="margin-top:14px">' +
      '<div class="setup-field" style="grid-column: 1 / -1;"><label>Player Name (Optional)</label><input type="text" id="unlisted_name_input" placeholder="e.g. Ray-Ray McCloud (Leave blank for Unlisted ' +
      unlistedSelectedPos +
      ')"></div>' +
  ...
  function submitUnlistedPick() {
    ...
    const bye = byeInput?.value ? parseInt(byeInput.value, 10) : null;
  ...
  function onLeagueSelectChange(newLeagueId) {
    const currentId =
      typeof global.getActiveLeagueId === 'function' ? global.getActiveLeagueId() : '';
  ...
  function handleDuplicateLeagueClick() {
    if (!confirmIfDirty()) return;
    const currentName = global.state?.settings?.leagueName || 'League';
  ...
  function handleDeleteLeagueClick() {
    ...
    const activeId =
      typeof global.getActiveLeagueId === 'function' ? global.getActiveLeagueId() : '';
    const currentName = global.state?.settings?.leagueName || 'this league';
    if (
      !confirm(
        `Are you sure you want to delete "${currentName}" and all of its draft picks? This action cannot be undone.`,
      )
    ) {
      return;
    }
        ...
        if (res?.ok) {
          ...
          alert(
            '✅ Successfully imported ' +
              (res.type === 'multi' ? `${res.count} leagues` : `league: ${res.name}`) +
              '!',
          );
          openLeagueSetup();
        } else {
  ...
  function openLeagueSetup() {
    ...
    const rs = Object.assign(
      {},
      typeof DEFAULT_ROSTER_SLOTS !== 'undefined' ? DEFAULT_ROSTER_SLOTS : {},
      s.rosterSlots,
    );
    ...
    const leagueManagerBarHtml =
      ...
      '<button type="button" class="act league-action-btn btn-delete" onclick="handleDeleteLeagueClick()"' +
      (leagues.length <= 1
        ? ' disabled title="Cannot delete the only remaining league"'
    ...
    function renderSetupTable(teamsCount) {
      ...
      for (let i = 1; i <= teamsCount; i++) {
        ...
        rows +=
          ...
          i +
          '">' +
          '<td class="slot-label"><span class="slot-badge">Slot ' +
          i +
          '</span></td>' +
          '<td><input type="text" class="team-input" id="team_input_' +
          i +
          '" data-slot="' +
          i +
          '" value="' +
          ...
          i +
          ' name"></td>' +
          '<td class="radio-cell"><label><input type="radio" name="setup_my_slot" value="' +
          i +
          '"' +
          (isMe ? ' checked' : '') +
          ' onchange="changeSetupMySlot(' +
          i +
          ')"> <span>My Team</span></label></td>' +
          '<td><div class="order-btns">' +
          '<button type="button" class="btn-move" title="Move Up" onclick="moveSetupTeam(' +
          i +
          ', -1)"' +
          (i === 1 ? ' disabled' : '') +
          '>▲</button>' +
          '<button type="button" class="btn-move" title="Move Down" onclick="moveSetupTeam(' +
          i +
          ', 1)"' +
          (i === teamsCount ? ' disabled' : '') +
          '>▼</button>' +
          '</div></td>' +
          '</tr>';
      }
    ...
    let espnSyncBtn = '';
    if (global.syncState?.espnLeagueInfo?.teams) {
      const info = global.syncState.espnLeagueInfo;
      espnSyncBtn =
        '<button type="button" class="act" onclick="applyEspnLeagueSetup(); openLeagueSetup();" style="font-size:11.5px; padding:3px 9px; color:var(--accent)">📥 Sync Teams & Slot from ESPN (' +
        info.teams +
        ' Teams' +
    ...
    $('modalbox').innerHTML =
      ...
      '<div class="setup-field" style="grid-column: 1 / -1;"><label>League / Board Name</label><input type="text" id="setup_league_name" value="' +
      (s.leagueName || 'Your Draft Board').replace(/"/g, '&quot;') +
      '"></div>' +
      '<div class="setup-field"><label>Total Teams</label><input type="number" id="setup_team_count" min="2" max="32" value="' +
      s.teams +
      '"></div>' +
      '<div class="setup-field"><label>Total Draft Rounds</label><input type="number" id="setup_rounds_count" min="1" max="50" value="' +
      s.rounds +
      '"></div>' +
      '<div class="setup-field"><label>Draft Order</label><select id="setup_mode_select"><option value="3rr"' +
      (s.mode === '3rr' ? ' selected' : '') +
      '>3rd-Round Reversal (3RR)</option><option value="snake"' +
      (s.mode === 'snake' ? ' selected' : '') +
      '>Normal Snake</option></select></div>' +
      '<div class="setup-field"><label>League Type</label><select id="setup_leaguetype_select"><option value="dynasty"' +
      (s.leagueType === 'dynasty' ? ' selected' : '') +
      '>Dynasty</option><option value="redraft"' +
      (s.leagueType === 'redraft' ? ' selected' : '') +
      '>Redraft</option></select></div>' +
      '<div class="setup-field"><label>Scoring Format</label><select id="setup_scoring_select"><option value="half"' +
      (s.scoring === 'half' ? ' selected' : '') +
      '>Half-PPR (0.5)</option><option value="ppr"' +
      (s.scoring === 'ppr' ? ' selected' : '') +
      '>Full PPR (1.0)</option><option value="std"' +
      (s.scoring === 'std' ? ' selected' : '') +
      '>Standard (0 PPR)</option></select></div>' +
      '<div class="setup-field"><label>QB Format</label><select id="setup_qb_select"><option value="sf"' +
      (s.qbFormat === 'sf' ? ' selected' : '') +
      '>Superflex (2QB/SF)</option><option value="1qb"' +
      (s.qbFormat === '1qb' ? ' selected' : '') +
      '>1 QB (Single QB)</option></select></div>' +
      '<div class="setup-field"><label>Max Keepers per Team</label><input type="number" id="setup_max_keepers" min="0" max="10" value="' +
      maxKeepersVal +
      '"></div>' +
      '</div>' +
      inSeasonSectionHtml +
      ...
      '<div class="meta" style="font-size:12px; margin-top:2px">Configure retained players assigned to each team and round. (Currently: <b style="color:var(--accent)">' +
      keeperCount +
      ' keeper' +
      (keeperCount === 1 ? '' : 's') +
      '</b> assigned)</div>' +
      '</div>' +
      '<button type="button" class="act primary" onclick="openKeepersModal()" style="font-weight:700; white-space:nowrap; padding:5px 14px">⭐ Manage Keepers (' +
      keeperCount +
      ')</button>' +
      '</div>' +
      ...
      '<div class="setup-field"><label>QB Starters</label><input type="number" id="setup_roster_qb" min="0" max="6" value="' +
      (rs.qb ?? 1) +
      '"></div>' +
      '<div class="setup-field"><label>RB Starters</label><input type="number" id="setup_roster_rb" min="0" max="8" value="' +
      (rs.rb ?? 2) +
      '"></div>' +
      '<div class="setup-field"><label>WR Starters</label><input type="number" id="setup_roster_wr" min="0" max="8" value="' +
      (rs.wr ?? 2) +
      '"></div>' +
      '<div class="setup-field"><label>TE Starters</label><input type="number" id="setup_roster_te" min="0" max="6" value="' +
      (rs.te ?? 1) +
      '"></div>' +
      '<div class="setup-field"><label>Flex (RB/WR/TE)</label><input type="number" id="setup_roster_flex" min="0" max="8" value="' +
      (rs.flex ?? 3) +
      '"></div>' +
      '<div class="setup-field"><label>Superflex (QB/RB/WR/TE)</label><input type="number" id="setup_roster_superflex" min="0" max="6" value="' +
      (rs.superflex ?? 1) +
      '"></div>' +
      '<div class="setup-field"><label>Kicker (K)</label><input type="number" id="setup_roster_k" min="0" max="4" value="' +
      (rs.k ?? 0) +
      '"></div>' +
      '<div class="setup-field"><label>Defense (D/ST)</label><input type="number" id="setup_roster_dst" min="0" max="4" value="' +
      (rs.dst ?? 0) +
      '"></div>' +
      '<div class="setup-field"><label>Bench Depth</label><input type="number" id="setup_roster_bench" min="0" max="40" value="' +
      (rs.bench ?? 15) +
      '"></div>' +
      '</div>' +
      ...
      '<div>📊 <b style="color:var(--text)">Rankings & Injury Status:</b> ' +
      (window.DRAFT_DATA?.players ? window.DRAFT_DATA.players.length : 0) +
      ' players loaded (Data date: <b style="color:var(--accent)">' +
      (window.DRAFT_DATA?.generated || 'live') +
      '</b>).</div>' +
    ...
    function updateRosterMath(fromRounds) {
      ...
      $('setup_roster_summary').innerHTML =
        '📋 <b style="color:var(--text)">Lineup:</b> ' +
        formatLineupSummary({ qb, rb, wr, te, flex, superflex: sf, k, dst, bench: currentBench }) +
        ' · <b style="color:var(--good)">' +
        starters +
        ' Starters</b> + <b style="color:var(--warn)">' +
        currentBench +
        ' Bench</b> = <b style="color:var(--accent)">' +
        totalRounds +
        ' Draft Rounds</b>';
    }
  ...
  function syncSetupInputsFromDom() {
    ...
    for (let i = 1; i <= count; i++) {
      ...
      if (input) {
        setupDraftNames[i - 1] =
          (input.value != null ? String(input.value).trim() : '') || `Team ${i}`;
      }
    }
  }
  ...
  async function triggerDataRefresh(btn) {
    ...
    if (statusEl) {
      ...
      statusEl.textContent =
        '⏳ Fetching latest consensus rankings, 32-team depth charts, and NFL injury reports (~10-15s)...';
    }
    ...
    try {
      ...
      if (statusEl) {
        ...
        statusEl.textContent =
          '⚠️ Live server refresh failed (' +
          err.message +
          '). Alternatively run "python scripts/update_rankings.py" in terminal.';
      }
      if (btn) {
        btn.disabled = false;
  ...
  function buildKeeperRoundOptions(slot, teams, rounds, mode, tradedPicks, keepers, editingId) {
    ...
    for (let r = 1; r <= rCount; r++) {
      ...
      const assigned = (keepers || []).filter(
        (k) => k && k.slot === slot && k.round === r && k.id !== editingId,
      ).length;
      if (owned.length === 0) {
        options +=
          '<option value="' +
          ...
          '" disabled>Round ' +
          r +
          ' (0 picks owned - traded away)</option>';
        ...
        options +=
          ...
          r +
          '"' +
          (assigned >= owned.length ? ' disabled' : '') +
          '>Round ' +
          r +
          ' (' +
          owned.length +
  ...
  function startEditKeeper(keeperId) {
    ...
    if (k.playerId != null) {
      keeperIsCustom = false;
      selectedKeeperPlayer = byId(k.playerId) || {
        id: k.playerId,
        name: `Player #${k.playerId}`,
        pos: 'WR',
        team: '',
      };
    } else {
  ...
  function renderKeepersModalView(explicitSlot) {
    ...
    const editingKeeper = editingKeeperId
      ? keepers.find((item) => item && item.id === editingKeeperId)
      : null;
    ...
    const roundOptions = buildKeeperRoundOptions(
      initialSlot,
      s.teams,
      s.rounds,
      s.mode,
    ...
    if (keepers.length === 0) {
      rowsHtml =
        '<tr><td colspan="6" style="text-align:center; padding:16px; color:var(--dim); font-style:italic">No keepers configured yet. Use the form above to add keeper players.</td></tr>';
    } else {
      ...
      for (const k of sorted) {
        ...
        const posClass = ['QB', 'RB', 'WR', 'TE'].includes(pos)
          ? pos
          : ['DST', 'DEF', 'D/ST'].includes(pos)
            ? 'DST'
            : pos === 'K'
              ? 'K'
              : 'other';
        ...
        rowsHtml +=
          ...
          '<button type="button" class="small" style="color:var(--accent); font-weight:600; cursor:pointer" onclick="startEditKeeper(\'' +
          k.id +
          '\')">✏️ Edit</button>' +
          '<button type="button" class="small" style="color:var(--bad); font-weight:600; cursor:pointer" onclick="handleRemoveKeeper(\'' +
          k.id +
          '\')">🗑️ Remove</button>' +
          '</div></td>' +
          '</tr>';
    ...
    const customPosButtons = ['QB', 'RB', 'WR', 'TE', 'K', 'DST', 'OTHER']
      .map(
        (pos) =>
          '<button type="button" class="tab' +
          (pos === keeperCustomPos ? ' on' : '') +
          '" onclick="selectKeeperCustomPos(\'' +
          pos +
          '\')">' +
          pos +
          '</button>',
      ...
      const posClass = ['QB', 'RB', 'WR', 'TE'].includes(p.pos)
        ...
        : ['DST', 'DEF', 'D/ST'].includes(p.pos)
          ...
          : p.pos === 'K'
            ...
            : 'other';
      selectedPillHtml =
        '<div style="display:inline-flex; align-items:center; gap:6px; background:var(--panel2); border:1px solid var(--good); padding:4px 8px; border-radius:6px; font-size:12px; margin-top:6px">' +
        '<span class="pos ' +
        posClass +
        '">' +
        p.pos +
    ...
    let formActionBtnHtml =
      '<button type="button" class="act primary" onclick="submitKeeperForm()" style="font-weight:700">Add Keeper</button>';
    if (editingKeeper) {
      const editingName =
        editingKeeper.customName ||
        (byId(editingKeeper.playerId)
          ? byId(editingKeeper.playerId).name
          : `Player #${editingKeeper.playerId}`);
      formHeaderHtml =
        '<div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:10px">' +
        '<h4 style="margin:0; color:var(--accent); font-size:13px; text-transform:none">✏️ Edit Keeper: <span style="color:var(--text)">' +
        editingName +
        '</span> (' +
        getTeamName(editingKeeper.slot) +
        ')</h4>' +
    ...
    const customNameVal = editingKeeper?.customName ? editingKeeper.customName : '';
    const customTeamVal = editingKeeper?.customTeam ? editingKeeper.customTeam : '';
    const customByeVal =
      editingKeeper && editingKeeper.customBye != null ? editingKeeper.customBye : '';
    ...
    $('modalbox').innerHTML =
      ...
      '<div><b style="color:var(--text)">Total Keepers Configured:</b> <span style="color:var(--accent); font-weight:700">' +
      keepers.length +
      '</span></div>' +
      '<div style="display:flex; align-items:center; gap:6px"><label style="font-size:12.5px; color:var(--dim)">Max Keepers per Team:</label> <input type="number" id="keeper_modal_max" min="0" max="10" style="width:54px; background:var(--bg); color:var(--text); border:1px solid var(--border); border-radius:4px; padding:2px 6px" value="' +
      maxK +
      '" onchange="handleKeeperMaxChange(this.value)"></div>' +
      '</div>' +
      ...
      '<div class="setup-field"><label>Team</label><select id="keeper_team_select" onchange="handleKeeperTeamChange()">' +
      teamOptions +
      '</select></div>' +
      '<div class="setup-field"><label>Draft Round Cost</label><select id="keeper_round_select">' +
      roundOptions +
      '</select></div>' +
      '</div>' +
      ...
      '<label style="font-size:12px; color:var(--dim); cursor:pointer"><input type="checkbox" id="keeper_custom_check" onchange="toggleKeeperCustom(this.checked)"' +
      (keeperIsCustom ? ' checked' : '') +
      '> Unlisted / Custom Player</label>' +
      '</div>' +
      '<div id="keeper_pool_search_wrap" style="' +
      (keeperIsCustom ? 'display:none;' : 'display:block;') +
      '" class="keeper-search-container">' +
      '<input type="text" id="keeper_search_input" placeholder="Search NFL player by name, team, position..." autocomplete="off" value="' +
      searchVal.replace(/"/g, '&quot;') +
      '" oninput="handleKeeperSearchInput(this.value)">' +
      '<div id="keeper_autocomplete" class="keeper-autocomplete-list" style="display:none"></div>' +
      '<div id="keeper_selected_pill">' +
      selectedPillHtml +
      '</div>' +
      '</div>' +
      '<div id="keeper_custom_wrap" style="' +
      (keeperIsCustom ? 'display:block;' : 'display:none;') +
      '">' +
      '<div class="tabs" style="flex-wrap:wrap; gap:4px; margin-bottom:8px" id="keeper_custom_pos_tabs">' +
      customPosButtons +
      '</div>' +
      '<div class="setup-grid" style="grid-template-columns: 2fr 1fr 1fr; gap:8px">' +
      '<div class="setup-field"><label>Player Name</label><input type="text" id="keeper_custom_name" placeholder="e.g. Travis Hunter" value="' +
      customNameVal.replace(/"/g, '&quot;') +
      '"></div>' +
      '<div class="setup-field"><label>NFL Team</label><input type="text" id="keeper_custom_team" placeholder="e.g. JAX" maxlength="4" value="' +
      customTeamVal.replace(/"/g, '&quot;') +
      '"></div>' +
      '<div class="setup-field"><label>Bye Week</label><input type="number" id="keeper_custom_bye" min="1" max="18" placeholder="e.g. 9" value="' +
      customByeVal +
      '"></div>' +
      '</div>' +
      '</div>' +
      ...
      '</div>' +
      '</div>' +
      '<h4 style="margin:16px 0 6px 0">📋 Current Keepers (' +
      keepers.length +
      ')</h4>' +
    ...
    if (editingKeeper?.round && $('keeper_round_select')) {
  ...
  function handleKeeperTeamChange() {
    ...
    const s = global.state.settings;
    const keepers = global.state.keepers || [];
    $('keeper_round_select').innerHTML = buildKeeperRoundOptions(
      slot,
      s.teams,
      s.rounds,
      s.mode,
  ...
  function toggleKeeperCustom(checked) {
    ...
    if ($('keeper_pool_search_wrap'))
      $('keeper_pool_search_wrap').style.display = keeperIsCustom ? 'none' : 'block';
    if ($('keeper_custom_wrap'))
      $('keeper_custom_wrap').style.display = keeperIsCustom ? 'block' : 'none';
  }

  function selectKeeperCustomPos(pos) {
    keeperCustomPos = pos;
    const buttons = ['QB', 'RB', 'WR', 'TE', 'K', 'DST', 'OTHER']
      .map(
        (p) =>
          '<button type="button" class="tab' +
          (p === keeperCustomPos ? ' on' : '') +
          '" onclick="selectKeeperCustomPos(\'' +
          p +
          '\')">' +
          p +
  ...
  function handleKeeperSearchInput(val) {
    ...
    const takenIds = new Set(
      (global.state.keepers || [])
        .filter((k) => k && k.id !== editingKeeperId)
        .map((k) => k.playerId)
        .filter((id) => id != null),
    );
    ...
    if (players.length === 0) {
      box.innerHTML =
        '<div style="padding:8px 10px; font-size:12px; color:var(--dim); font-style:italic">No matching players in pool</div>';
      ...
      const posClass = ['QB', 'RB', 'WR', 'TE'].includes(p.pos)
        ...
        : ['DST', 'DEF', 'D/ST'].includes(p.pos)
          ...
          : p.pos === 'K'
            ...
            : 'other';
      itemsHtml +=
        '<div class="keeper-autocomplete-item" onclick="selectKeeperSearchPlayer(' +
        p.id +
        ')">' +
        '<span class="pos ' +
        posClass +
  ...
  function submitKeeperForm() {
    ...
    if (keeperIsCustom) {
      ...
      if (!selectedKeeperPlayer) {
        if (errBox) {
          errBox.textContent =
            'Please search and select a player from the pool, or check Unlisted / Custom Player';
          errBox.style.display = 'block';
        }
        return;
      }
  ...
  function copyLastDraftPick() {
    ...
    const fmtFn =
      typeof formatPickForClipboard === 'function'
        ? formatPickForClipboard
        : typeof window !== 'undefined' && typeof window.formatPickForClipboard === 'function'
          ? window.formatPickForClipboard
          : null;
    const text =
      typeof fmtFn === 'function'
