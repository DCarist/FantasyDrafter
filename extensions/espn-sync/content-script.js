...
(function () {
  ...
  function isPlaceholderName(name, teamNames = []) {
    ...
    // Reject if name matches or contains any identified league team name
    const allTeams = [...(teamNames || []), ...(detectedTeamNames || [])];
    for (const t of allTeams) {
      if (t && typeof t === 'string') {
        const tClean = t.trim().toLowerCase();
        if (tClean) {
          if (clean === tClean || clean.includes(tClean) || (clean.length >= 4 && tClean.includes(clean))) {
            return true;
          }
        }
      }
  ...
  function scanDraftRoom(force) {
    ...
    completedCells.forEach(cell => {
      ...
      if (!name || isPlaceholderName(name, detectedTeamNames)) return;
