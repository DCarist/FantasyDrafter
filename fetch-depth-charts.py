...
Usage:
    python fetch-depth-charts.py [--out-js players-data.js] [--out-json players-data.json]
"""
...
def fetch_team_depth_chart(team_abbr, espn_code, lookup_exact, lookup_name):
    ...
    off = next(
        (
            d
            for d in data.get("depthchart", [])
            if "qb" in d.get("positions", {})
        ),
        None,
    )
    sp = next(
        (
            d
            for d in data.get("depthchart", [])
            if "pk" in d.get("positions", {})
        ),
        None,
    )

    def parse_athletes(pos_container, pos_key, role=None):
        if not pos_container or pos_key not in pos_container.get(
            "positions", {}
        ):
            return []
        athletes = []
