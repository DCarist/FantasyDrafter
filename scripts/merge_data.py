...
def main():
    ...
    out_target = (
        os.path.join("data", "players-data.js") if os.path.exists("data") else "players-data.js"
    )
    ...
    print(
        f"bye map teams: {len(byes)}; team-name conflicts (majority vote used): {team_conflicts}; pos conflicts: {pos_conflicts}"
    )
    ...
    for p in top:
        print(
            f"  {p['dynSF']:>3} {p['name']:<24} {p['pos']:<3} {p['team'] or '?':<4} bye {p['bye'] or '?'} age {p['age'] or '?'}"
        )
